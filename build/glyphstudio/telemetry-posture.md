# GlyphStudio — Telemetry Posture

> Honest statement of what GlyphStudio does and does not send, what was **verified**
...
> ledger entry and `build/glyphstudio/vendored-telemetry-allowlist.json` for the
> per-file inventory.

## Claim

**GlyphStudio introduces no telemetry. Telemetry, the Microsoft Marketplace, and the
Microsoft update service are disabled at the product level. The vendored upstream
built-ins ship dormant Microsoft endpoint constants that GlyphStudio does not
activate.**

This is a security CLAIM. The static gate proves the *negative* part mechanically
(no GlyphStudio-introduced telemetry string; the only endpoint strings are an
...
## What GlyphStudio introduces — VERIFIED zero telemetry

Verified 2026-05-30 against the packaged app
(`../VSCode-darwin-arm64/GlyphStudio.app`) and `product.json`:

- **Embedded `glyphstudio-trust-panel` extension:** appears in **none** of the gate's
...
- **Issue reporter:** `reportIssueUrl` = the GlyphStudio repo
  (`github.com/corynaegle-ai/GlyphStudioIDE/issues/new`), not Microsoft's.
- **Webview CDN:** `webviewContentExternalBaseUrlTemplate` is empty (see
  `build/glyphstudio/webview-cdn-removal.md`).

If any GlyphStudio-authored surface (the embedded extension, `product.json`, or new
...
telemetry/Marketplace/update endpoints as **string constants**. GlyphStudio ships
...
`build/glyphstudio/verify-no-telemetry.sh` greps the packaged app for the HARD
...
- hit path matches nothing → **UNEXPECTED** → **hard FAIL** (exit 1). This catches
  GlyphStudio-introduced telemetry *and* any new vendored file not yet inventoried.
...
bash build/glyphstudio/verify-no-telemetry.sh ../VSCode-darwin-arm64/GlyphStudio.app
