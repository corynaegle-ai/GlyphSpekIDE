#!/usr/bin/env bash
# GlyphStudio IDE — platform app-icon generator.
#
# Input : a SQUARE master PNG (>= 1024x1024 recommended).
#         default: resources/glyphstudio/glyphstudio-icon-master.png
...
#
# NOTE: the current master is the wordmark-FREE GlyphStudio G-mark, cropped so the mark
# fills ~73% of the 1024 canvas for a strong Dock presence at small sizes. The prior
# master is kept alongside as glyphstudio-icon-master-prev.png for fallback. To adjust the
# mark size, re-crop the master and re-run this script.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
MASTER="${1:-$ROOT/resources/glyphstudio/glyphstudio-icon-master.png}"

[ -f "$MASTER" ] || { echo "error: master not found: $MASTER" >&2; exit 1; }

TMP="$(mktemp -d)"
ICONSET="$TMP/glyphstudio.iconset"
mkdir -p "$ICONSET"
...
if [ -n "$ICO" ]; then
  ...
  echo "      brew install imagemagick && build/glyphstudio/generate-icons.sh   # to refresh the .ico" >&2
fi
