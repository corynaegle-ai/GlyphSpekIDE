# GlyphStudio — Copilot onboarding wizard must no-op (fatal blank-window fix)
...
GlyphStudio's M1 branding stripped the GitHub Copilot `defaultChatAgent` block from
`product.json` (GlyphStudio ships no Copilot). But the Copilot-onboarding wizard
...
- `toDefaultAccountConfig(defaultChatAgent)` dereferenced `defaultChatAgent.chatExtensionId`
  ...
  `productService.defaultChatAgent`, which is `undefined` in GlyphStudio → renderer blank.
...
- `getProductBuiltInExtensionsEnabledWithAutoUpdates()` did
  `for (const id of productService.builtInExtensionsEnabledWithAutoUpdates)`. GlyphStudio's
...
- Rebuilt `vscode-darwin-arm64`, relaunched `GlyphStudio.app`: the workbench now **paints**
