...
> **first-party GlyphStudio extension** (NOT in the Code-OSS workbench; no fork patch).
> Lives in the extension bundle (`extension/` → IDE extension), not under `src/vs/`.
> Nothing here is wired or compiled.

```ts
// glyphstudio-extension: src/posture/ambientExtensionWarning.ts  (NOT YET CREATED)
import * as vscode from 'vscode';

const FIRST_PARTY_PUBLISHER = 'glyphstudio';
...
function readPosture(): Posture {
  return vscode.workspace
    .getConfiguration('glyphstudio.extensions')
...
export function registerAmbientExtensionWarning(ctx: vscode.ExtensionContext) {
  ...
  const evaluate = () => {
    ...
    if (ambient.length > 0 && key !== warnedKey) {
      ...
      vscode.window.showWarningMessage(
        `GlyphStudio is in Developer mode. ${ambient.length} third-party extension(s) ` +
        `run outside GlyphStudio's trust boundary. Governed agent runs remain isolated, ` +
  ...
  ctx.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(e => {
      if (e.affectsConfiguration('glyphstudio.extensions')) { evaluate(); }
    }),
  );
  evaluate();
}
...
```jsonc
"glyphstudio.extensions.mode":          { "type": "string", "enum": ["sovereign","developer"], "default": "developer" },
"glyphstudio.extensions.ambientWarning":{ "type": "boolean", "default": true }
```

These are GlyphStudio-owned settings, distinct from stock `extensions.allowed`. Their default
values are also published via the Sovereign/Developer overlays in this folder.

## NOT wired

- No `glyphstudio-extension` source file exists yet; this is shape-only.
