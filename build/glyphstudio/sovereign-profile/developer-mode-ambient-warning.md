...
This warning is **GlyphStudio's responsibility (first-party extension)** — it is not a stock
...
2. at least one **enabled** extension is **not** in the first-party set
   (`publisher === "glyphstudio"`) and **not** a built-in/system extension
...
3. **One-time status/notification:** on first detection per window session, an informational
   (not modal) notification: *"GlyphStudio is in Developer mode. N third-party extension(s)
   are enabled and run outside GlyphStudio's trust boundary. Governed agent runs remain
