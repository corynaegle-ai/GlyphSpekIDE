...
`extensions/glyphstudio-trust-panel/`. It is **built out-of-band** from the
...
```sh
bash build/glyphstudio/sync-embedded-extension.sh
```

This is the **only** supported way to update `extensions/glyphstudio-trust-panel/`.
...
`extensions/**/dist/` is gitignored, but `extensions/glyphstudio-trust-panel/dist/**`
