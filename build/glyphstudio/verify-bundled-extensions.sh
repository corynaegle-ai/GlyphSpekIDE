#!/usr/bin/env bash
# GlyphStudio — bundled-extension deny gate.
#
# WHAT THIS CHECKS
...
#       build/glyphstudio/bundled-extension-denylist.json. A clean GlyphStudio build
...
#   (B) Chains build/glyphstudio/verify-no-copilot.sh, which scans the WHOLE app
...
#   (C) Chains build/glyphstudio/verify-embedded-extension.sh — the source<->embedded
#       conformance gate (command IDs, activationEvents, dist/*.js,
#       dist-supervisor/*.mjs, pinned supervisor hashes).
#   (D) Chains build/glyphstudio/sync-embedded-extension.sh --check --no-build — the
...
#
# USAGE
#   build/glyphstudio/verify-bundled-extensions.sh [TARGET_DIR]
#
...
MANIFEST="$REPO_ROOT/build/glyphstudio/bundled-extension-denylist.json"
...
echo "=== GlyphStudio bundled-extension deny gate ==="
echo "target:     $TARGET"
...
COPILOT_GATE="$REPO_ROOT/build/glyphstudio/verify-no-copilot.sh"
...
# The bundled extensions/glyphstudio-trust-panel/ is a vendored snapshot of the
...
EMBEDDED_GATE="$REPO_ROOT/build/glyphstudio/verify-embedded-extension.sh"
...
# embedded copy.
media_sync_fail=0
SYNC_CHECK="$REPO_ROOT/build/glyphstudio/sync-embedded-extension.sh"
...
XTERM_GATE="$REPO_ROOT/build/glyphstudio/verify-vendored-xterm.sh"
...
if [ "$violations" -gt 0 ] || [ "$copilot_runtime_fail" -gt 0 ] || [ "$embedded_conformance_fail" -gt 0 ] || [ "$media_sync_fail" -gt 0 ] || [ "$vendored_xterm_fail" -gt 0 ]; then
  if [ "$violations" -gt 0 ]; then
    ...
    echo "        GlyphStudio must not ship these. Remove them from the build/package path."
  fi
  if [ "$copilot_runtime_fail" -gt 0 ]; then
    echo "RESULT: FAIL — Copilot RUNTIME libraries present in the packaged app"
    echo "        (see the no-Copilot-runtime gate above). GlyphStudio must ship no Copilot."
  fi
  if [ "$embedded_conformance_fail" -gt 0 ]; then
    echo "RESULT: FAIL — the embedded GlyphStudio extension drifted from its source"
    echo "        (see the source<->embedded conformance gate above). Re-sync with the"
    echo "        canonical command (do NOT copy files by hand):"
    echo "            bash build/glyphstudio/sync-embedded-extension.sh"
  fi
  if [ "$media_sync_fail" -gt 0 ]; then
    echo "RESULT: FAIL — the embedded GlyphStudio media/** tree (Trust Panel webview:"
