#!/usr/bin/env bash
# GlyphStudio — reproducible source -> fork-embedded extension sync.
#
# WHY THIS EXISTS
#   The fork embeds a VENDORED COPY of the first-party extension at
#   extensions/glyphstudio-trust-panel/. That copy is built out-of-band from the
...
#
#   It is the inverse of build/glyphstudio/verify-embedded-extension.sh: the verify
...
#
# USAGE
#   build/glyphstudio/sync-embedded-extension.sh [--check] [--no-build] [SOURCE_DIR]
#
...
#     SOURCE_DIR   Override the source tree (default: $GLYPHSTUDIO_EXTENSION_SOURCE,
...
EMBEDDED="$REPO_ROOT/extensions/glyphstudio-trust-panel"

if [ -z "$SOURCE" ] || [ ! -d "$SOURCE" ]; then
	echo "sync-embedded-extension: SOURCE extension tree not found." >&2
	echo "  Looked for: \$GLYPHSTUDIO_EXTENSION_SOURCE, ../extension, ../../extension" >&2
...
echo "=== GlyphStudio source -> embedded extension sync [$mode] ==="
echo "source:   $SOURCE"
...
if [ "$CHECK_ONLY" -eq 1 ]; then
	if [ "$diffs" -gt 0 ]; then
		...
		echo
		echo "  Re-sync with: bash build/glyphstudio/sync-embedded-extension.sh"
		exit 1
	fi
...
echo
echo "Next: bash build/glyphstudio/verify-bundled-extensions.sh   (conformance gate)"
exit 0
