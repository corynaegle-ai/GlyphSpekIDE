#!/usr/bin/env bash
# GlyphStudio — source <-> fork-embedded extension conformance gate (Sweep-20 F2).
#
# WHAT THIS CHECKS
#   The fork embeds a VENDORED COPY of the first-party extension at
#   extensions/glyphstudio-trust-panel/. That copy is built/bundled out-of-band
...
#   commands (e.g. glyphstudio.startLiveRun), a renamed dist/ file set, or a
...
#
# USAGE
#   build/glyphstudio/verify-embedded-extension.sh [SOURCE_DIR] [EMBEDDED_DIR]
#
#   SOURCE_DIR   defaults to the first that exists:
#                  $GLYPHSTUDIO_EXTENSION_SOURCE
#                  ../extension                 (sibling to repo root)
#                  ../../extension
#   EMBEDDED_DIR defaults to extensions/glyphstudio-trust-panel (in this repo).
#
# EXIT CODES
...
EMBEDDED="${2:-$REPO_ROOT/extensions/glyphstudio-trust-panel}"
...
echo "=== GlyphStudio source <-> embedded conformance gate (Sweep-20 F2) ==="
echo "source:   $SOURCE"
...
if [ "$drift" -gt 0 ]; then
	...
	echo "        the fork-embedded copy at extensions/glyphstudio-trust-panel/."
