# GlyphStudio — Removing the Microsoft-CDN webview dependency
...
**Removing/emptying `webviewContentExternalBaseUrlTemplate` is safe for the GlyphStudio desktop
...
**web/browser** workbench build, which GlyphStudio does not ship. We emptied the field (rather
...
- **Web/browser:**
  ...
  anyway — emptying the field does not, by itself, remove the CDN from a web build. GlyphStudio
...
- Set `webviewContentExternalBaseUrlTemplate` to `""` in `product.json`. This removes the last
  genuine `vscode-cdn.net` runtime endpoint string from the GlyphStudio product config without
...
2. **If GlyphStudio ever ships a web/server build:** the empty product field will fall back to the
