#!/usr/bin/env bash
# GlyphStudio — M1 "telemetry-free startup" verifier.
#
# WHAT THIS CHECKS
#   (A) STATIC: greps the BUILT app bundle (or the dev `out/` dir) for known
#       Microsoft telemetry + Marketplace + CDN endpoints. A clean GlyphStudio
...
#
# USAGE
#   build/glyphstudio/verify-no-telemetry.sh [TARGET_DIR]
#
#   TARGET_DIR defaults, in order, to the first that exists:
#     ../VSCode-darwin-arm64/GlyphStudio.app   (packaged build, relative to repo root)
#     ./out                                   (dev build)
#
...
#   (build/glyphstudio/vendored-telemetry-allowlist.json):
...
#       description). GlyphStudio introduces no telemetry; telemetry is OFF at the
...
#     - GlyphStudio-INTRODUCED / UNEXPECTED: a HARD hit whose path is NOT in the
#       allowlist (e.g. the embedded glyphstudio-trust-panel extension, product.json,
#       or any new/GlyphStudio-authored code). This HARD-FAILS — GlyphStudio must
...
#   1  = at least one UNEXPECTED HARD hit (GlyphStudio-introduced or not yet
...
ALLOWLIST_JSON="$REPO_ROOT/build/glyphstudio/vendored-telemetry-allowlist.json"
VENDORED_ALLOW=()
...
if [ -z "$TARGET" ]; then
  for cand in \
    "$REPO_ROOT/../VSCode-darwin-arm64/GlyphStudio.app" \
    "$REPO_ROOT/out"; do
...
if [ -z "$TARGET" ] || [ ! -e "$TARGET" ]; then
  ...
  echo "  Looked for: ../VSCode-darwin-arm64/GlyphStudio.app  and  ./out" >&2
  echo "  Pass an explicit path: $0 /path/to/GlyphStudio.app" >&2
  exit 2
fi

echo "=== GlyphStudio telemetry static scan ==="
echo "target: $TARGET"
...
if [ "$TARGET_IS_PACKAGED" -ne 1 ]; then
  ...
  echo "      ../VSCode-darwin-arm64/GlyphStudio.app for the authoritative verdict."
fi
echo
...
if [ "$unexpected_hits" -eq 0 ] && [ "$vendored_hits" -eq 0 ]; then
  ...
  echo "OK: $vendored_hits vendored-dormant hit(s), all inventoried; 0 unexpected/GlyphStudio-introduced."
fi
echo
...
offline-first launch:
  ...
  1) Quit any running GlyphStudio; clear its data dir so first-run paths fire:
       rm -rf "$HOME/.glyphstudio" "$HOME/Library/Application Support/GlyphStudio"
  ...
  2) Start a packet capture filtered to the suspect hosts (needs sudo). Run in
     a second terminal BEFORE launching the app:
       sudo tcpdump -n -i any -q \
         ...
         | tee /tmp/glyphstudio-startup-capture.txt
  ...
  3) Launch the app and let it sit on the welcome screen ~60s, then quit:
       open ../VSCode-darwin-arm64/GlyphStudio.app
  ...
  "visualstudio" or "vscode-cdn"), or `nettop -m tcp -p GlyphStudio`.
EOF
echo

# ---- bundled-extension deny gate --------------------------------------------
# Chain the bundled-extension deny gate: a clean GlyphStudio build must also not
...
bundled_ext_fail=0
DENY_GATE="$REPO_ROOT/build/glyphstudio/verify-bundled-extensions.sh"
if [ -x "$DENY_GATE" ]; then
...
echo "      GlyphStudio product.json sets NO aiConfig/enableTelemetry/aiKey, no"
...
echo "      build/glyphstudio/vendored-telemetry-allowlist.json (confirmed string"
echo "      constants / localized doc-link text, not active call paths); an UNEXPECTED"
echo "      hit is a GlyphStudio-introduced or not-yet-inventoried telemetry string and"
...
if [ "$bundled_ext_fail" -gt 0 ]; then
  ...
  echo "        (see the bundled-extension deny gate above). GlyphStudio must not ship them."
  exit 1
fi
if [ "$unexpected_hits" -gt 0 ]; then
  ...
  echo "        finding: it is either GlyphStudio-INTRODUCED telemetry (must be zero) or a"
...
if [ "$vendored_hits" -gt 0 ]; then
  echo "RESULT: PASS — GlyphStudio introduces ZERO telemetry. $vendored_hits HARD endpoint"
  echo "        hit(s) are all VENDORED-DORMANT (inventoried in"
  echo "        build/glyphstudio/vendored-telemetry-allowlist.json) — Microsoft-authored"
