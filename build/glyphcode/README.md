...
`extensions/glyphcode-trust-panel/`. It is **built out-of-band** from the
...
```sh
bash build/glyphcode/sync-embedded-extension.sh
```

This is the **only** supported way to update `extensions/glyphcode-trust-panel/`.
...
`extensions/**/dist/` is gitignored, but `extensions/glyphcode-trust-panel/dist/**`
