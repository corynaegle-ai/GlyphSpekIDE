...
> folder are meant to reach a produced GlyphCode artifact, so M1 implementation has a target.
...
2. **macOS:** deliver the `AllowedExtensions` value via managed prefs for
   `dev.glyphcode.app` (MDM, or `defaults write`). Stock `NativePolicyService`
   (`main.ts:220-221`).
3. **Windows:** deliver via registry/GPO under `GlyphCode` (`win32RegValueName`). Stock
   `NativePolicyService` (`main.ts:218-219`).
4. **Any platform, self-contained:** launch with `--__enable-file-policy` and place the file
   at `~/.glyphcode/policy.json` (`environmentService.ts:267-274`). For a shrink-wrapped app
...
4. fail the build if a curated entry uses bare `true` for a non-`glyphcode` publisher.
