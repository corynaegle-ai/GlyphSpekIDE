...
> **first-party GlyphCode extension** (NOT in the Code-OSS workbench; no fork patch).
> Lives in the extension bundle (`extension/` → IDE extension), not under `src/vs/`.
> Nothing here is wired or compiled.

```ts
// glyphcode-extension: src/posture/ambientExtensionWarning.ts  (NOT YET CREATED)
import * as vscode from 'vscode';

const FIRST_PARTY_PUBLISHER = 'glyphcode';
...
function readPosture(): Posture {
  return vscode.workspace
    .getConfiguration('glyphcode.extensions')
...
export function registerAmbientExtensionWarning(ctx: vscode.ExtensionContext) {
  ...
  const evaluate = () => {
    ...
    if (ambient.length > 0 && key !== warnedKey) {
      ...
      vscode.window.showWarningMessage(
        `GlyphCode is in Developer mode. ${ambient.length} third-party extension(s) ` +
        `run outside GlyphCode's trust boundary. Governed agent runs remain isolated, ` +
  ...
  ctx.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(e => {
      if (e.affectsConfiguration('glyphcode.extensions')) { evaluate(); }
    }),
  );
  evaluate();
}
...
```jsonc
"glyphcode.extensions.mode":          { "type": "string", "enum": ["sovereign","developer"], "default": "developer" },
"glyphcode.extensions.ambientWarning":{ "type": "boolean", "default": true }
```

These are GlyphCode-owned settings, distinct from stock `extensions.allowed`. Their default
values are also published via the Sovereign/Developer overlays in this folder.

## NOT wired

- No `glyphcode-extension` source file exists yet; this is shape-only.
