# GlyphCode — Removing the Microsoft-CDN webview dependency
...
**Removing/emptying `webviewContentExternalBaseUrlTemplate` is safe for the GlyphCode desktop
...
**web/browser** workbench build, which GlyphCode does not ship. We emptied the field (rather
...
- **Web/browser:**
  ...
  anyway — emptying the field does not, by itself, remove the CDN from a web build. GlyphCode
...
- Set `webviewContentExternalBaseUrlTemplate` to `""` in `product.json`. This removes the last
  genuine `vscode-cdn.net` runtime endpoint string from the GlyphCode product config without
...
2. **If GlyphCode ever ships a web/server build:** the empty product field will fall back to the
