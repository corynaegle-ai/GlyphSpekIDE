#!/usr/bin/env bash
# GlyphCode — source <-> fork-embedded extension conformance gate (Sweep-20 F2).
#
# WHAT THIS CHECKS
#   The fork embeds a VENDORED COPY of the first-party extension at
#   extensions/glyphcode-trust-panel/. That copy is built/bundled out-of-band
...
#   commands (e.g. glyphcode.startLiveRun), a renamed dist/ file set, or a
...
#
# USAGE
#   build/glyphcode/verify-embedded-extension.sh [SOURCE_DIR] [EMBEDDED_DIR]
#
#   SOURCE_DIR   defaults to the first that exists:
#                  $GLYPHCODE_EXTENSION_SOURCE
#                  ../extension                 (sibling to repo root)
#                  ../../extension
#   EMBEDDED_DIR defaults to extensions/glyphcode-trust-panel (in this repo).
#
# EXIT CODES
...
EMBEDDED="${2:-$REPO_ROOT/extensions/glyphcode-trust-panel}"
...
echo "=== GlyphCode source <-> embedded conformance gate (Sweep-20 F2) ==="
echo "source:   $SOURCE"
...
if [ "$drift" -gt 0 ]; then
	...
	echo "        the fork-embedded copy at extensions/glyphcode-trust-panel/."
