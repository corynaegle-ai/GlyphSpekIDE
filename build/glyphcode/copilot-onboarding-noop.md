# GlyphCode — Copilot onboarding wizard must no-op (fatal blank-window fix)
...
GlyphCode's M1 branding stripped the GitHub Copilot `defaultChatAgent` block from
`product.json` (GlyphCode ships no Copilot). But the Copilot-onboarding wizard
...
- `toDefaultAccountConfig(defaultChatAgent)` dereferenced `defaultChatAgent.chatExtensionId`
  ...
  `productService.defaultChatAgent`, which is `undefined` in GlyphCode → renderer blank.
...
- `getProductBuiltInExtensionsEnabledWithAutoUpdates()` did
  `for (const id of productService.builtInExtensionsEnabledWithAutoUpdates)`. GlyphCode's
...
- Rebuilt `vscode-darwin-arm64`, relaunched `GlyphCode.app`: the workbench now **paints**
