# GlyphCode — M4 Escape-Hatch Discovery & Classification

> **Deliverable:** M4 security task — enumerate every surface across the Code-OSS fork
> (`glyphcode-codeoss`, branch `glyphcode`), the first-party extension
> (`extensions/glyphcode-trust-panel/` ← source `../extension`), and the supervisor
> (`spikes/p0-supervisor`) that lets code RUN or EGRESS *outside* GlyphCode's
...
GlyphCode's core claim: it GOVERNS an agent's egress (metadata-only proxy), produces
...
`glyphcode` branch from its first GlyphCode commit, the GlyphCode-introduced changes to
...
stock Code-OSS.** (Verified: `git diff <first-glyphcode-commit> HEAD -- src/vs/**` shows
no terminal/task/debug/tunnel/workspaceTrust files touched.)

GlyphCode's *governance* — the egress proxy, sanitized env, trace, signed verdict —
...
**the GlyphCode envelope is a per-run/per-terminal envelope inside an otherwise stock
...
1. **A1 — the un-governed stock integrated terminal inside a "GlyphCode IDE".** The
...
3. **E2 — Remote-Tunnels / `serve-web` shipped unmodified.** A stock tunnel opens a
   ...
   governance envelope and could connect to a non-GlyphCode service. High-value to gate
...
- **CIO-FLAG-1 — "GlyphCode IDE" is a per-run envelope inside a stock IDE, not a
  ...
  that GlyphCode governs *the runs/terminals it creates*, not the whole IDE, and with the
...
> This addendum audits exactly those **GlyphCode-ADDED** surfaces under the same governance
...
> surfaces GlyphCode built and brands as its AI DX. An ungoverned one is a self-inflicted
...
| **G8** | **Chat LM provider as a public proxy** (another extension `selectChatModels({vendor:'glyphcode'})` → `sendRequest`). | NO | **PATCHED (fail-closed)** | `provideLanguageModelChatResponse` THROWS on every direct call (`chatParticipant.ts:1077-1096`); the model exists only to satisfy the host default-model resolver. |
