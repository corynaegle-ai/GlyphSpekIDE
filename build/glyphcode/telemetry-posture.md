# GlyphCode — Telemetry Posture

> Honest statement of what GlyphCode does and does not send, what was **verified**
...
> ledger entry and `build/glyphcode/vendored-telemetry-allowlist.json` for the
> per-file inventory.

## Claim

**GlyphCode introduces no telemetry. Telemetry, the Microsoft Marketplace, and the
Microsoft update service are disabled at the product level. The vendored upstream
built-ins ship dormant Microsoft endpoint constants that GlyphCode does not
activate.**

This is a security CLAIM. The static gate proves the *negative* part mechanically
(no GlyphCode-introduced telemetry string; the only endpoint strings are an
...
## What GlyphCode introduces — VERIFIED zero telemetry

Verified 2026-05-30 against the packaged app
(`../VSCode-darwin-arm64/GlyphCode.app`) and `product.json`:

- **Embedded `glyphcode-trust-panel` extension:** appears in **none** of the gate's
...
- **Issue reporter:** `reportIssueUrl` = the GlyphCode repo
  (`github.com/corynaegle-ai/GlyphCodeIDE/issues/new`), not Microsoft's.
- **Webview CDN:** `webviewContentExternalBaseUrlTemplate` is empty (see
  `build/glyphcode/webview-cdn-removal.md`).

If any GlyphCode-authored surface (the embedded extension, `product.json`, or new
...
telemetry/Marketplace/update endpoints as **string constants**. GlyphCode ships
...
`build/glyphcode/verify-no-telemetry.sh` greps the packaged app for the HARD
...
- hit path matches nothing → **UNEXPECTED** → **hard FAIL** (exit 1). This catches
  GlyphCode-introduced telemetry *and* any new vendored file not yet inventoried.
...
bash build/glyphcode/verify-no-telemetry.sh ../VSCode-darwin-arm64/GlyphCode.app
