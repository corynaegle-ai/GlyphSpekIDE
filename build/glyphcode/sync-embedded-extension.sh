#!/usr/bin/env bash
# GlyphCode — reproducible source -> fork-embedded extension sync.
#
# WHY THIS EXISTS
#   The fork embeds a VENDORED COPY of the first-party extension at
#   extensions/glyphcode-trust-panel/. That copy is built out-of-band from the
...
#
#   It is the inverse of build/glyphcode/verify-embedded-extension.sh: the verify
...
#
# USAGE
#   build/glyphcode/sync-embedded-extension.sh [--check] [--no-build] [SOURCE_DIR]
#
...
#     SOURCE_DIR   Override the source tree (default: $GLYPHCODE_EXTENSION_SOURCE,
...
EMBEDDED="$REPO_ROOT/extensions/glyphcode-trust-panel"

if [ -z "$SOURCE" ] || [ ! -d "$SOURCE" ]; then
	echo "sync-embedded-extension: SOURCE extension tree not found." >&2
	echo "  Looked for: \$GLYPHCODE_EXTENSION_SOURCE, ../extension, ../../extension" >&2
...
echo "=== GlyphCode source -> embedded extension sync [$mode] ==="
echo "source:   $SOURCE"
...
if [ "$CHECK_ONLY" -eq 1 ]; then
	if [ "$diffs" -gt 0 ]; then
		...
		echo
		echo "  Re-sync with: bash build/glyphcode/sync-embedded-extension.sh"
		exit 1
	fi
...
echo
echo "Next: bash build/glyphcode/verify-bundled-extensions.sh   (conformance gate)"
exit 0
