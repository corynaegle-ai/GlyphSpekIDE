#!/usr/bin/env bash
# GlyphSpek IDE — platform app-icon generator.
#
# Input : a SQUARE master PNG (>= 1024x1024 recommended).
#         default: resources/glyphspek/glyphspek-icon-master.png
...
MASTER="${1:-$ROOT/resources/glyphspek/glyphspek-icon-master.png}"

[ -f "$MASTER" ] || { echo "error: master not found: $MASTER" >&2; exit 1; }

TMP="$(mktemp -d)"
ICONSET="$TMP/glyphspek.iconset"
mkdir -p "$ICONSET"
...
if [ -n "$ICO" ]; then
  ...
  echo "      brew install imagemagick && build/glyphspek/generate-icons.sh   # to refresh the .ico" >&2
fi
