# GlyphSpek — Sovereign vs Developer Extension Profile
...
Code-OSS already ships a complete, layered allowlist that GlyphSpek can drive entirely from
configuration:
...
loader prefer a GlyphSpek-bundled `policy.json` on macOS/Windows so the Sovereign policy
...
- **macOS / Windows:** `NativePolicyService` reading managed preferences / registry under
  **our own** identity (`darwinBundleIdentifier = dev.glyphspek.app`,
  `win32RegValueName = GlyphSpek`, from `product.json`). MDM/Intune/GPO or a `defaults
  write dev.glyphspek.app` can set it.
- **Portable / explicit file (all platforms):** passing `--__enable-file-policy` makes
  `environmentService.policyFile` resolve to `~/.glyphspek/policy.json` (or
...
1. **No patch (deployment-only):** ship the Sovereign policy via OS managed prefs (MDM/GPO)
   or document the `--__enable-file-policy` launch flag + `~/.glyphspek/policy.json`. Fully
...
2. **`PATCH-001` (recommended for shrink-wrapped Sovereign builds):** a narrow change to the
   *policy-source selection* in `main.ts` / `cliProcessMain.ts` so that, when a
   GlyphSpek-bundled policy file exists (or `product.json` carries a sovereign flag), the
...
| Required UI | Posture badge = "Sovereign"; disallowed-ext notification (stock). | **Ambient-extension warning** when non-first-party extensions are enabled (GlyphSpek extension; see below). |

- **Selection:** chosen at provisioning time. A Sovereign build/deployment ships the
  policy; a Developer build does not. The first-party GlyphSpek extension reads the
...
- **Detecting posture from the extension:** the extension determines posture by inspecting
  ...
  `glyphspek.extensions.mode`** (a GlyphSpek-owned setting we also publish via the same
...
```jsonc
{
  // first-party: pin GlyphSpek publisher entirely
  "glyphspek": true,
...
| `sovereign-policy.json` | Generated **policy-file** form (`{ "AllowedExtensions": {…} }`) for `FilePolicyService` (`/etc/vscode/policy.json`, `~/.glyphspek/policy.json`, or MDM-equivalent). | **stub** |
| `default-settings.sovereign.json` | Sovereign **default-settings overlay** (telemetry off, gallery = Open VSX, autoUpdate off, `glyphspek.extensions.mode=sovereign`). For the workbench default-config overlay, NOT user settings. | **stub** |
...
3. **`glyphspek.extensions.mode` setting is not registered** anywhere in core/extension yet.
