...
> folder are meant to reach a produced GlyphSpek artifact, so M1 implementation has a target.
...
2. **macOS:** deliver the `AllowedExtensions` value via managed prefs for
   `dev.glyphspek.app` (MDM, or `defaults write`). Stock `NativePolicyService`
   (`main.ts:220-221`).
3. **Windows:** deliver via registry/GPO under `GlyphSpek` (`win32RegValueName`). Stock
   `NativePolicyService` (`main.ts:218-219`).
4. **Any platform, self-contained:** launch with `--__enable-file-policy` and place the file
   at `~/.glyphspek/policy.json` (`environmentService.ts:267-274`). For a shrink-wrapped app
...
4. fail the build if a curated entry uses bare `true` for a non-`glyphspek` publisher.
