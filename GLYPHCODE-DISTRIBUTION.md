# GlyphCode — Clean Code‑OSS Distribution Plan
...
- Fork: `corynaegle-ai/GlyphCodeIDE` (fork of `microsoft/vscode`; repo renamed from `vscode` → `GlyphCodeIDE`).
- This working tree: shallow clone (`git clone --depth 1`) of the fork, now at `glyph-code/glyphcode-codeoss/` (inside the planning repo, gitignored; IDE code commits to the fork's own remote).
- **M0 status (2026-05-30):** branding applied on branch `glyphcode` — `product.json` rebranded, Open VSX gallery wired, Copilot/telemetry blocks stripped, app icons regenerated from `resources/glyphcode/` (macOS `.icns` + Linux `.png` done; Windows `.ico` pending ImageMagick via `build/glyphcode/generate-icons.sh`).
...
For the GlyphCode distribution, override **at minimum** these fields. Use the existing
GlyphCode naming conventions (`glyphcode` publisher/short name, `.glyphcode` data folder,
no Microsoft Marketplace, no telemetry) already established in `../glyph-code/extension/`.

| `product.json` field | Upstream value | GlyphCode distribution value | Purpose |
| --- | --- | --- | --- |
| `nameShort` | `Code - OSS` | `GlyphCode` | Short product name (window title, About) |
| `nameLong` | `Code - OSS` | `GlyphCode` | Long product name |
| `applicationName` | `code-oss` | `glyphcode` | CLI binary name / `bin` entry |
| `serverApplicationName` | `code-server-oss` | `glyphcode-server` | Remote server binary name |
| `serverDataFolderName` | `.vscode-server-oss` | `.glyphcode-server` | Server-side data dir |
| `tunnelApplicationName` | `code-tunnel-oss` | `glyphcode-tunnel` | Tunnel CLI name |
| `dataFolderName` | `.vscode-oss` | `.glyphcode` | Per-user data dir (`~/.glyphcode`) — matches extension keystore root |
| `sharedDataFolderName` | `.vscode-oss-shared` | `.glyphcode-shared` | Shared data dir |
| `urlProtocol` | `code-oss` | `glyphcode` | Custom URL scheme (`glyphcode://`) |
| `darwinBundleIdentifier` | `com.visualstudio.code.oss` | `dev.glyphcode.app` (TBD) | macOS bundle id |
| `linuxIconName` | `code-oss` | `glyphcode` | Linux icon basename |
| `win32DirName` / `win32NameVersion` / `win32RegValueName` / `win32AppUserModelId` / `win32MutexName` / `win32ShellNameShort` | `…Code OSS…` | `GlyphCode` variants | Windows install/registry identity (regen the `win32*AppId` GUIDs) |
| `reportIssueUrl` | `…/microsoft/vscode/issues/new` | `https://github.com/corynaegle-ai/glyph-code/issues/new` | "Report Issue" target |
| `licenseUrl` / `serverLicenseUrl` | upstream MIT links | GlyphCode repo `LICENSE` links | License links |
...
> GlyphCode extension already declares "no telemetry"; the distribution must not reintroduce
...
1. **Edit `product.json` in place** on a `glyphcode` branch of the fork (simplest; the file
   *is* the intended override point). This is the recommended P0 path.
2. **Overlay file + build copy** — keep a `product.glyphcode.json` and copy it over
...
```bash
# from this clone root: /Users/cory.naegle/Projects/glyph-spek/glyphcode-codeoss
