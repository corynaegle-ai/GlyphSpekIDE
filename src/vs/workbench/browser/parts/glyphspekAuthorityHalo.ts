...
import './media/glyphspekAuthorityHalo.css';
...
/**
 * GlyphSpek Authority Halo (PATCH-003).
 *
 ...
 *
 * Mechanism (read-only): the GlyphSpek first-party extension sets the
 * `glyphspek.authority` context-key (`read | claimed | soft | verified | denied`,
 * default `read`) on every assurance change. This contribution observes that key
 * and mirrors it to a `data-glyphspek-authority` attribute on the `.monaco-workbench`
...
const AUTHORITY_ATTRIBUTE = 'data-glyphspek-authority';

/** The class that opts the workbench root into the floating-frame substrate look. */
const FLOATING_FRAME_CLASS = 'glyphspek-floating-frame';

/** The class that disables the halo motion (pulse + cross-fade), keeping static color. */
const REDUCE_MOTION_CLASS = 'glyphspek-halo-reduce-motion';

const FLOATING_FRAME_SETTING = 'glyphspek.workbench.floatingFrame';
const HALO_MOTION_SETTING = 'glyphspek.workbench.haloMotion';
...
export class GlyphSpekAuthorityHaloContribution extends Disposable {

	static readonly ID = 'workbench.contrib.glyphspekAuthorityHalo';
	...
	constructor(
		...
		// React to the two GlyphSpek workbench settings.
	...
	private applyAuthority(authority: string): void {
		...
		if (authority === 'denied' && previous !== 'denied') {
			this.root.classList.remove('glyphspek-halo-deny-pulse');
			// Force a reflow so the animation can be re-triggered.
			void this.root.offsetWidth;
			this.root.classList.add('glyphspek-halo-deny-pulse');
	...
	override dispose(): void {
		...
		root.classList.remove(FLOATING_FRAME_CLASS, REDUCE_MOTION_CLASS, 'glyphspek-halo-deny-pulse');
		super.dispose();
	}
}

registerWorkbenchContribution2(GlyphSpekAuthorityHaloContribution.ID, GlyphSpekAuthorityHaloContribution, WorkbenchPhase.AfterRestored);

// GlyphSpek workbench settings backing the halo + floating frame.
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	...
	properties: {
		[FLOATING_FRAME_SETTING]: {
			...
			description: localize('glyphspek.workbench.floatingFrame', "Float the workbench on a dark substrate with rounded corners so the Authority Halo ring has somewhere to glow. The halo ring reflects run assurance regardless of this setting.")
		},
		[HALO_MOTION_SETTING]: {
			type: 'boolean',
			default: true,
			scope: ConfigurationScope.APPLICATION,
			description: localize('glyphspek.workbench.haloMotion', "Animate Authority Halo transitions (the calm color cross-fade and the one-shot deny pulse). When off, the halo keeps its static assurance color with no animation. The OS \"reduce motion\" preference also disables these animations.")
		}
	}
});
