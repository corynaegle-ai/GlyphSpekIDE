...
import './media/glyphcodeTitleLadder.css';
...
/**
 * GlyphCode Title-bar Authority Ladder (PATCH-009) — the canonical FRICTION control.
 *
 ...
 *   - FRICTION (this widget's job): the active RUNG is driven by `glyphcode.tier`.
 *   - ASSURANCE (borrowed for TINT ONLY): the level chip's color + label come from
 *     `glyphcode.authority` (the same halo color map). The ladder NEVER sets, derives,
 ...
 * promote modal (glyphcode.promoteChatToBuild) remains the only authority door. The
 * native rung click invokes the `glyphcode.setTier` COMMAND (the single writer of the
 * `glyphcode.tier` context-key, which also syncs the webview ladder). This widget never
...
const SET_TIER_COMMAND = 'glyphcode.setTier';

/** The setting that toggles the whole widget (default ON). */
const TITLE_LADDER_SETTING = 'glyphcode.workbench.titleLadder';

/** Neutral, honest actor placeholder when no actor is bound (no fabricated name). */
const NO_ACTOR_LABEL = localize('glyphcode.actor.none', "No actor bound");
...
const TIER_LABELS: Record<Tier, string> = {
	ask: localize('glyphcode.ladder.ask', "Ask"),
	inline: localize('glyphcode.ladder.inline', "Inline Edit"),
	governed: localize('glyphcode.ladder.governed', "Governed Run"),
	sensitive: localize('glyphcode.ladder.sensitive', "Sensitive"),
	sovereign: localize('glyphcode.ladder.sovereign', "Sovereign")
};
...
const AUTHORITY_CHIP: Record<string, { glyph: string; label: string }> = {
	read: { glyph: '•', label: localize('glyphcode.lvl.read', "read · no authority yet") },
	claimed: { glyph: '◇', label: localize('glyphcode.lvl.claimed', "claimed · self-reported, unverified") },
	soft: { glyph: '◈', label: localize('glyphcode.lvl.soft', "SOFT · governed-unsandboxed") },
	verified: { glyph: '✓', label: localize('glyphcode.lvl.verified', "verified · signed") },
	denied: { glyph: '✕', label: localize('glyphcode.lvl.denied', "UNTRUSTED · denied") }
};

export class GlyphCodeTitleLadderContribution extends Disposable {

	static readonly ID = 'workbench.contrib.glyphcodeTitleLadder';
	...
	private mount(center: HTMLElement): void {
		...
		const root = append(center, $('.glyphcode-title-ladder'));
		this.container = root;
		...
		this.actorMini = append(root, $('span.glyphcode-tl-actor'));
		this.actorMini.setAttribute('aria-hidden', 'false');

		// The ladder radiogroup (ARIA mirrors the webview ladder's pattern).
		const ladder = append(root, $('.glyphcode-tl-ladder'));
		ladder.setAttribute('role', 'radiogroup');
		ladder.setAttribute('aria-label', localize('glyphcode.ladder.aria', "Authority ladder — choose which evidence to view (does not grant authority)"));

		this.rungs = new Map<Tier, HTMLElement>();
		for (const tier of KNOWN_TIERS) {
			const rung = append(ladder, $('button.glyphcode-tl-rung'));
		...
		this.widgetDisposables.add(addDisposableListener(ladder, EventType.CLICK, e => {
			...
			const rung = target?.closest?.('.glyphcode-tl-rung') as HTMLElement | null;
		...
		this.levelChip = append(root, $('span.glyphcode-tl-level'));
	 ...
	 * extension's `glyphcode.setTier` command validates the tier, sets the context-key
	...
	private requestTier(tier: Tier): void {
		...
		this.commandService.executeCommand(SET_TIER_COMMAND, tier).then(undefined, err => {
			...
			this.logService.trace(`[glyphcode] ${SET_TIER_COMMAND} unavailable (extension activating?): ${err}`);
		});
	}
	...
	private render(): void {
		...
		// attribute so the companion CSS picks the right --glyphcode-tl-halo tint.
		this.container.setAttribute('data-glyphcode-tier', this.currentTier);
		this.container.setAttribute('data-glyphcode-authority', this.currentAuthority);
		...
		if (this.actorMini) {
			...
			this.actorMini.classList.toggle('glyphcode-tl-actor-none', actor === NO_ACTOR_LABEL);
		}
	}
	...
	private readBoundActor(): string {
		const raw = this.contextKeyService.getContextKeyValue<string>('glyphcode.actor');
...
registerWorkbenchContribution2(GlyphCodeTitleLadderContribution.ID, GlyphCodeTitleLadderContribution, WorkbenchPhase.AfterRestored);

// GlyphCode workbench setting backing the title-bar Authority Ladder. Registered under
// the same `glyphcode` configuration id as the halo + tier-chrome settings so they group.
