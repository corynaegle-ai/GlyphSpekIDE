...
import './media/glyphspekTierChrome.css';
...
/**
 * GlyphSpek Tier Chrome (PATCH-008) — the FRICTION axis, sibling to the Authority Halo.
 *
 ...
 * (§2.1): this file NEVER reads or writes `glyphspek.authority`, and the halo never
 * reads `glyphspek.tier`. They are kept in separate files so that separation is legible
 * at the source level.
 *
 ...
 *
 * Mechanism (read-only, mirrors the halo exactly): the GlyphSpek first-party extension
 * sets the `glyphspek.tier` context-key on every effective-tier change (its Trust Panel
 * Authority Ladder is a VIEW control; the promote modal remains the only authority door).
 * This contribution observes that key and mirrors it to a `data-glyphspek-tier` attribute
...
const TIER_ATTRIBUTE = 'data-glyphspek-tier';

/** The class that opts the editor-recede out (so the user can disable the cue). */
const RECEDE_OFF_CLASS = 'glyphspek-tier-recede-off';

// REDUCED MOTION is handled entirely in the companion CSS: the OS `prefers-reduced-motion`
// media query AND the shared `glyphspek-halo-reduce-motion` class that the Authority Halo
// contribution toggles from the `glyphspek.workbench.haloMotion` setting. A single user/OS
// preference therefore quiets BOTH GlyphSpek chrome motions, so this contribution does not
// re-toggle that class itself (the halo owns it) — it just relies on it in CSS.

const ASK_RECEDE_SETTING = 'glyphspek.workbench.askEditorRecede';
...
export class GlyphSpekTierChromeContribution extends Disposable {

	static readonly ID = 'workbench.contrib.glyphspekTierChrome';
...
registerWorkbenchContribution2(GlyphSpekTierChromeContribution.ID, GlyphSpekTierChromeContribution, WorkbenchPhase.AfterRestored);

// GlyphSpek workbench setting backing the Ask editor-recede. Registered under the same
// `glyphspek` configuration id as the halo settings so they group in Settings UI.
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	...
	properties: {
		[ASK_RECEDE_SETTING]: {
			...
			description: localize('glyphspek.workbench.askEditorRecede', "At the Ask friction tier, let the center editor recede (grayscale, dimmed, with a translucent scrim) so the lowest-authority tier feels light. This is a purely visual friction cue — it never alters layout, never traps interaction, and never implies a trust or assurance state.")
		}
	}
});
