...
import './media/glyphspekTitleLadder.css';
...
/**
 * GlyphSpek Title-bar Authority Ladder (PATCH-009) — the canonical FRICTION control.
 *
 ...
 *   - FRICTION (this widget's job): the active RUNG is driven by `glyphspek.tier`.
 *   - ASSURANCE (borrowed for TINT ONLY): the level chip's color + label come from
 *     `glyphspek.authority` (the same halo color map). The ladder NEVER sets, derives,
 ...
 * promote modal (glyphspek.promoteChatToBuild) remains the only authority door. The
 * native rung click invokes the `glyphspek.setTier` COMMAND (the single writer of the
 * `glyphspek.tier` context-key, which also syncs the webview ladder). This widget never
...
/** The context-key the GlyphSpek extension writes the effective friction tier to. */
const TIER_CONTEXT_KEY = 'glyphspek.tier';

/** The context-key the GlyphSpek extension writes the focused-run assurance to. */
const AUTHORITY_CONTEXT_KEY = 'glyphspek.authority';

/** The command (owned by the extension) that is the SINGLE writer of the tier key. */
const SET_TIER_COMMAND = 'glyphspek.setTier';

/** The setting that toggles the whole widget (default ON). */
const TITLE_LADDER_SETTING = 'glyphspek.workbench.titleLadder';

/** Neutral, honest actor placeholder when no actor is bound (no fabricated name). */
const NO_ACTOR_LABEL = localize('glyphspek.actor.none', "No actor bound");
...
const TIER_LABELS: Record<Tier, string> = {
	ask: localize('glyphspek.ladder.ask', "Ask"),
	inline: localize('glyphspek.ladder.inline', "Inline Edit"),
	governed: localize('glyphspek.ladder.governed', "Governed Run"),
	sensitive: localize('glyphspek.ladder.sensitive', "Sensitive"),
	sovereign: localize('glyphspek.ladder.sovereign', "Sovereign")
};
...
const AUTHORITY_CHIP: Record<string, { glyph: string; label: string }> = {
	read: { glyph: '•', label: localize('glyphspek.lvl.read', "read · no authority yet") },
	claimed: { glyph: '◇', label: localize('glyphspek.lvl.claimed', "claimed · self-reported, unverified") },
	soft: { glyph: '◈', label: localize('glyphspek.lvl.soft', "SOFT · governed-unsandboxed") },
	verified: { glyph: '✓', label: localize('glyphspek.lvl.verified', "verified · signed") },
	denied: { glyph: '✕', label: localize('glyphspek.lvl.denied', "UNTRUSTED · denied") }
};

export class GlyphSpekTitleLadderContribution extends Disposable {

	static readonly ID = 'workbench.contrib.glyphspekTitleLadder';
	...
	private mount(center: HTMLElement): void {
		...
		const root = append(center, $('.glyphspek-title-ladder'));
		this.container = root;
		...
		this.actorMini = append(root, $('span.glyphspek-tl-actor'));
		this.actorMini.setAttribute('aria-hidden', 'false');

		// The ladder radiogroup (ARIA mirrors the webview ladder's pattern).
		const ladder = append(root, $('.glyphspek-tl-ladder'));
		ladder.setAttribute('role', 'radiogroup');
		ladder.setAttribute('aria-label', localize('glyphspek.ladder.aria', "Authority ladder — choose which evidence to view (does not grant authority)"));

		this.rungs = new Map<Tier, HTMLElement>();
		for (const tier of KNOWN_TIERS) {
			const rung = append(ladder, $('button.glyphspek-tl-rung'));
		...
		this.widgetDisposables.add(addDisposableListener(ladder, EventType.CLICK, e => {
			...
			const rung = target?.closest?.('.glyphspek-tl-rung') as HTMLElement | null;
		...
		this.levelChip = append(root, $('span.glyphspek-tl-level'));
	 ...
	 * extension's `glyphspek.setTier` command validates the tier, sets the context-key
	...
	private requestTier(tier: Tier): void {
		...
		this.commandService.executeCommand(SET_TIER_COMMAND, tier).then(undefined, err => {
			...
			this.logService.trace(`[glyphspek] ${SET_TIER_COMMAND} unavailable (extension activating?): ${err}`);
		});
	}
	...
	private render(): void {
		...
		// attribute so the companion CSS picks the right --glyphspek-tl-halo tint.
		this.container.setAttribute('data-glyphspek-tier', this.currentTier);
		this.container.setAttribute('data-glyphspek-authority', this.currentAuthority);
		...
		if (this.actorMini) {
			...
			this.actorMini.classList.toggle('glyphspek-tl-actor-none', actor === NO_ACTOR_LABEL);
		}
	}
	...
	private readBoundActor(): string {
		const raw = this.contextKeyService.getContextKeyValue<string>('glyphspek.actor');
...
registerWorkbenchContribution2(GlyphSpekTitleLadderContribution.ID, GlyphSpekTitleLadderContribution, WorkbenchPhase.AfterRestored);

// GlyphSpek workbench setting backing the title-bar Authority Ladder. Registered under
// the same `glyphspek` configuration id as the halo + tier-chrome settings so they group.
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	id: 'glyphspek',
	order: 8,
	title: localize('glyphspekWorkbenchConfigurationTitle', "GlyphSpek"),
	type: 'object',
	properties: {
