...
export interface IChatViewWelcomeContent {
	...
	/**
	 * GlyphStudio: optional browser URI of a full-color raster logo rendered as an
	...
	constructor(
		...
		try {
			...
			if (content.iconImageUri) {
				// GlyphStudio: render the full-color brand logo as a raster `<img>`.
				// No `currentColor`/mask treatment so brand colors are preserved.
				icon.classList.add('glyphstudio-logo');
