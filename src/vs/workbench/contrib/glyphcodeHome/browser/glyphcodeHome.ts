...
import './media/glyphcodeHome.css';
...
import { GlyphspekHomeInput } from './glyphcodeHomeInput.js';
import { createGlyphspekHomeIconSprite } from './glyphcodeHomeIcons.js';
import {
 ...
 * the GlyphCode extension is not active (its commands unregistered), the row renders visibly
 * disabled with a "needs the GlyphCode extension" hint rather than a live-looking no-op.
 */
...
/** The primary governed-run command (extension: "GlyphCode: Build This (Governed Run)"). */
const BUILD_COMMAND_ID = 'glyphcode.promoteChatToBuild';

/**
 * The governance command list. Each id is a command CONTRIBUTED by the GlyphCode extension
...
const HOME_COMMANDS: readonly IHomeCommand[] = [
	{
		commandId: 'glyphcode.openTrustPanel',
		label: localize('glyphcodeHome.openTrustPanel', "Open Trust Panel"),
		description: localize('glyphcodeHome.openTrustPanel.desc', "Evidence, not transcript — claims vs. signed verdict."),
		icon: 'gs-verifier'
	},
	{
		commandId: 'glyphcode.runs.openTrustPanel',
		label: localize('glyphcodeHome.openVerifier', "Open Independent Verifier"),
		description: localize('glyphcodeHome.openVerifier.desc', "Re-runs declared checks in a clean trust domain and signs a verdict."),
		icon: 'gs-broker'
	},
	{
		commandId: 'workbench.view.extension.glyphcode-egress',
		label: localize('glyphcodeHome.openEgress', "Open Egress / Network"),
		description: localize('glyphcodeHome.openEgress.desc', "Observe-and-trace egress (metadata-only) for governed runs — allowed-this-run is recorded, not blocked."),
		icon: 'gs-network'
	},
	{
		commandId: 'workbench.view.extension.glyphcode-trace',
		label: localize('glyphcodeHome.openTrace', "Open Trace / Provenance"),
		description: localize('glyphcodeHome.openTrace.desc', "The hash-chained, tamper-evident event chain for the focused run."),
		icon: 'gs-trace'
	},
	{
		commandId: 'workbench.action.findInFiles',
		label: localize('glyphcodeHome.search', "Search"),
		description: localize('glyphcodeHome.search.desc', "Find across the workspace."),
		icon: 'gs-search'
	}
];
 ...
 *
 * Renders the blended-workbench aesthetic (floating frame, halo palette, GlyphCode wordmark)
 * as a LIVE, HONEST surface: the primary input starts a REAL governed run via the extension's
 * `glyphcode.promoteChatToBuild` command (typed task → build intent arg); each governance row
...
export class GlyphspekHomePage extends EditorPane {
	...
	static readonly ID = 'glyphcodeHomePage';
	...
	protected createEditor(parent: HTMLElement): void {
		this.container = $('.glyphcode-home', {
			role: 'document',
			tabindex: 0,
			'aria-label': localize('glyphcodeHome.aria', "GlyphCode Home — start a governed run and reach the governance surfaces.")
		});
		...
		try {
			...
			console.error('[GlyphCode Home] icon sprite injection failed; rendering without brand icons', err);
		}
		append(parent, this.container);

		// Re-render the command rows when the GlyphCode extension (de)registers its commands,
	...
	private renderBody(): void {
		...
		const scroll = append(this.container, $('.glyphcode-home-scroll'));
		const inner = append(scroll, $('.glyphcode-home-inner'));
	...
	private renderHeader(parent: HTMLElement): void {
		...
		title.textContent = 'GlyphCode';
		const tagline = append(header, $('.gsh-tagline'));
		tagline.textContent = localize('glyphcodeHome.tagline', "Agent-safe engineering workstation — more autonomy, less blind trust.");
	}
	...
	private renderDraftComposer(parent: HTMLElement): void {
		...
		headText.textContent = localize('glyphcodeHome.startTitle', "Start a governed run");

		const sub = append(card, $('.gsh-card-sub'));
		sub.textContent = localize('glyphcodeHome.startSub', "Describe a task. The governed agent acts inside the supervisor envelope — it WILL edit files and run commands, every step brokered and traced.");

		const inputRow = append(card, $('.gsh-input-row'));
		const textarea = append(inputRow, $('textarea.gsh-task-input')) as HTMLTextAreaElement;
		textarea.placeholder = localize('glyphcodeHome.startPlaceholder', "e.g. \"add input validation to the signup form and a test for it\"");
		textarea.rows = 3;
		textarea.setAttribute('aria-label', localize('glyphcodeHome.startAria', "Describe the task for the governed agent to build"));
		...
		buttonLabel.textContent = localize('glyphcodeHome.startButton', "Start governed run");
		...
		if (buildAvailable) {
			...
			hintText.textContent = localize('glyphcodeHome.startHintLive', "Submits to the governed build (you approve authority up front). Press Enter to start, Shift+Enter for a newline.");
		} else {
			append(hint, this.icon('gs-broker', 'gsh-hint-icon gsh-hint-muted'));
			const hintText = append(hint, $('span.gsh-hint-muted'));
			hintText.textContent = localize('glyphcodeHome.startHintUnavailable', "The GlyphCode extension is not active in this window, so governed runs can't start yet.");
		}
	}
	...
	private renderSelectedRunComposer(parent: HTMLElement): void {
		...
		append(head, this.icon('gs-run', 'gsh-head-icon'));
		const headText = append(head, $('span'));
		headText.textContent = localize('glyphcodeHome.composerSelectedTitle', "Selected run");
		...
		if (typeof intentText === 'string' && intentText.length > 0) {
			...
			intentBox.textContent = detail
				? localize('glyphcodeHome.composerNoIntent', "untitled run — no intent text was recorded.")
				: localize('glyphcodeHome.composerIntentLoading', "Loading the run's intent…");
		}
		...
		statusLabel.textContent = localize('glyphcodeHome.composerStatusLabel', "status");
		const statusValue = append(statusRow, $('span.gsh-composer-status-value'));
		if (detail) {
			statusValue.textContent = detail.closed
				? localize('glyphcodeHome.composerStatusClosed', "{0} (closed)", detail.status)
				: detail.status;
		} else {
			statusValue.textContent = localize('glyphcodeHome.composerStatusLoading', "loading…");
		}
		...
		append(hint, this.icon('gs-broker', 'gsh-hint-icon'));
		const hintText = append(hint, $('span'));
		hintText.textContent = localize('glyphcodeHome.composerSelectedHint', "Read-only — this run is already governed. Its evidence is on the right. Clear the selection to start a new run.");

		const back = append(card, $('button.gsh-composer-back')) as HTMLButtonElement;
		back.textContent = localize('glyphcodeHome.composerBack', "Start a new run instead");
	...
	private renderCommands(parent: HTMLElement): void {
		...
		sectionHead.textContent = localize('glyphcodeHome.commandsTitle', "Governance surfaces");
	...
	private renderCommandRow(parent: HTMLElement, cmd: IHomeCommand): void {
		...
		desc.textContent = available
			? cmd.description
			: localize('glyphcodeHome.rowUnavailable', "Needs the GlyphCode extension active in this window.");
	...
	private renderRunsList(parent: HTMLElement): void {
		...
		const sectionHead = append(section, $('.gsh-section-head'));
		sectionHead.textContent = localize('glyphcodeHome.runsTitle', "Runs");
	...
	private renderRunsEmpty(parent: HTMLElement): void {
		...
		if (!this.runProvider.isExtensionActive()) {
			emptyTitle.textContent = localize('glyphcodeHome.runsExtensionInactive', "GlyphCode extension not active");
			emptySub.textContent = localize('glyphcodeHome.runsExtensionInactiveSub', "Runs are owned by the GlyphCode extension. Activate it in this window to see governed runs here.");
		} else {
			emptyTitle.textContent = localize('glyphcodeHome.runsEmpty', "No runs yet — start one in the center.");
			emptySub.textContent = localize('glyphcodeHome.runsEmptySub', "Governed runs you start appear here; selecting one shows its evidence on the right.");
		}
	}
	...
	private renderRunRow(parent: HTMLElement, run: AgentRun): void {
		...
		if (run.closed) {
			...
			closedEl.textContent = localize('glyphcodeHome.runClosed', "closed");
		}
	...
	private statePillLabel(state: RunCardState): string {
		switch (state) {
			case 'verified': return localize('glyphcodeHome.pillVerified', "verified");
			case 'blocked': return localize('glyphcodeHome.pillBlocked', "blocked");
			default: return localize('glyphcodeHome.pillActing', "acting");
		}
	}
	 ...
	 * `src/vs/workbench/browser/parts/glyphcodeAuthorityHalo.ts` mirrors the
	 * `glyphcode.authority` CONTEXT-KEY (which the EXTENSION sets for the FOCUSED run) onto a
	 * `data-glyphcode-authority` attribute on `.monaco-workbench`, and the companion CSS paints
	...
	private applyAuthorityTint(): void {
		...
		if (level) {
			...
			panes.title = localize('glyphcodeHome.authorityTitle', "Authority halo: {0}", authorityLabel(level));
		} else {
	 ...
	 * run's AgentRunDetail (fetched via the provider's `glyphcode.runs.detail` consumer).
	...
	private renderEvidencePane(parent: HTMLElement): void {
		const section = append(parent, $('.gsh-section'));
		const sectionHead = append(section, $('.gsh-section-head'));
		sectionHead.textContent = localize('glyphcodeHome.evidenceTitle', "Evidence");
	...
	private renderEvidenceBody(): void {
		...
		if (!this.selectedRunId) {
			this.renderEvidencePlaceholder(
				body,
				localize('glyphcodeHome.evidenceNone', "Select a run to see its evidence."),
				localize('glyphcodeHome.evidenceNoneSub', "Its posture, changes, and signed verdict appear here.")
			);
			return;
		}
		...
		if (!detail) {
			...
			if (!this.runProvider.isExtensionActive()) {
				this.renderEvidencePlaceholder(
					body,
					localize('glyphcodeHome.evidenceInactive', "GlyphCode extension not active"),
					localize('glyphcodeHome.evidenceInactiveSub', "Run evidence is owned by the GlyphCode extension. Activate it in this window to load it here.")
				);
			} else {
				this.renderEvidencePlaceholder(
					body,
					localize('glyphcodeHome.evidenceLoading', "Loading evidence…"),
					localize('glyphcodeHome.evidenceLoadingSub', "Fetching the selected run's posture, changes, and verdict.")
				);
			}
			return;
		}
	...
	private renderEvidenceHeader(parent: HTMLElement, detail: AgentRunDetail): void {
		...
		statusEl.textContent = detail.closed
			? localize('glyphcodeHome.evidenceStatusClosed', "{0} (closed)", detail.status)
			: detail.status;
	...
	private renderPostureSection(parent: HTMLElement, detail: AgentRunDetail): void {
		const body = this.openEvidenceSection(parent, 'gs-sandbox', localize('glyphcodeHome.evPosture', "Posture"));
	...
	private renderChangesSection(parent: HTMLElement, detail: AgentRunDetail): void {
		const body = this.openEvidenceSection(parent, 'gs-policy', localize('glyphcodeHome.evChanges', "Changes"));
		if (detail.changedFiles.length === 0 && detail.diff.trim().length === 0) {
			const none = append(body, $('.gsh-ev-muted'));
			none.textContent = localize('glyphcodeHome.evNoChanges', "No changes recorded for this run yet.");
			return;
		}
		...
		if (detail.diff.trim().length > 0) {
			...
			caption.textContent = localize('glyphcodeHome.evDiffCaption', "Review surface — the unified diff of the run's changes (not a sandbox).");
		}
	}
	...
	private renderVerdictSection(parent: HTMLElement, detail: AgentRunDetail): void {
		const body = this.openEvidenceSection(parent, 'gs-verifier', localize('glyphcodeHome.evVerdict', "Verdict"));
		const verdict = detail.verdict;
		if (!verdict) {
			// Honest "no verdict yet" — never a fabricated PASS. Distinct from "select a run".
			const none = append(body, $('.gsh-ev-muted'));
			none.textContent = localize('glyphcodeHome.evNoVerdict', "No evidence yet — this run has no signed verifier verdict.");
			return;
		}
		...
		assuranceEl.textContent = verdict.assurance === 'full'
			? localize('glyphcodeHome.evAssuranceFull', "assurance: full")
			: localize('glyphcodeHome.evAssuranceDegraded', "assurance: degraded");

		// --- signature state (presence ≠ trust) ---
		const sigEl = append(verdictCard, $('.gsh-ev-signature'));
		if (blueEligible) {
			sigEl.classList.add('gsh-ev-sig-verified');
			sigEl.textContent = localize('glyphcodeHome.evSigVerified', "signature verified · product-trusted");
			...
			sigEl.textContent = detail.verified
				...
				? localize('glyphcodeHome.evSigSoftCap', "signature present · capped by soft posture (not product-trusted)")
				: localize('glyphcodeHome.evSigUnverified', "signature present · not verified");
		} else {
			sigEl.classList.add('gsh-ev-sig-none');
			sigEl.textContent = localize('glyphcodeHome.evSigNone', "no signature");
		}
		...
		if (verdict.verifierIsolation || verdict.verifyCommandSource) {
			...
			if (verdict.verifierIsolation) {
				...
				iso.textContent = verdict.verifierIsolation === 'independent-sandboxed'
					? localize('glyphcodeHome.evIsoIndependent', "verifier: independent-sandboxed")
					: localize('glyphcodeHome.evIsoInline', "verifier: inline-unsandboxed");
			}
			if (verdict.verifyCommandSource) {
				...
				src.textContent = localize('glyphcodeHome.evCmdSource', "checks: {0}", verdict.verifyCommandSource);
			}
		}
		...
		if (verdict.traceRootHash) {
			...
			rootLabel.textContent = localize('glyphcodeHome.evTraceRoot', "trace root");
	...
	private renderCheck(parent: HTMLElement, check: AgentRunCheck): void {
		...
		status.textContent = check.status === 'skipped'
			? localize('glyphcodeHome.evCheckSkipped', "skipped")
			: check.status;
	}
...
function postureLabel(posture: CreationTrustPosture): string {
	switch (posture) {
		case 'trusted': return localize('glyphcodeHome.postureTrusted', "Trusted (pending gate)");
		case 'sandboxed-soft-egress': return localize('glyphcodeHome.postureSandboxedSoft', "Sandboxed (soft egress)");
		case 'governed-unsandboxed': return localize('glyphcodeHome.postureGovernedUnsandboxed', "Governed (soft) — unsandboxed");
		case 'untrusted': return localize('glyphcodeHome.postureUntrusted', "Untrusted");
		case 'refused': return localize('glyphcodeHome.postureRefused', "Refused");
		default: return localize('glyphcodeHome.postureUnknown', "Starting…");
	}
}
...
function postureFraming(posture: CreationTrustPosture): string {
	switch (posture) {
		case 'trusted':
			return localize('glyphcodeHome.postureFramingTrusted', "Hard isolation + hard egress reported. Eligible for a product-trusted verdict ONLY after the signature gate passes.");
		case 'sandboxed-soft-egress':
			return localize('glyphcodeHome.postureFramingSandboxedSoft', "SOFT — not contained. Isolation runtime, but the egress boundary is soft. Never product-trusted.");
		case 'governed-unsandboxed':
			return localize('glyphcodeHome.postureFramingGovernedUnsandboxed', "SOFT — not contained. Governed + traced via the metadata-only egress proxy, but unsandboxed. Never product-trusted.");
		case 'untrusted':
			return localize('glyphcodeHome.postureFramingUntrusted', "No trusted runtime could be established. Cannot be product-trusted.");
		case 'refused':
			return localize('glyphcodeHome.postureFramingRefused', "The supervisor refused to create the run. No trusted run was produced.");
		default:
			return localize('glyphcodeHome.postureFramingUnknown', "Trust posture not settled yet (no RunOpened observed).");
	}
}
...
function verdictOverallLabel(overall: 'pass' | 'fail' | 'error', tone: 'blue' | 'amber' | 'red'): string {
	switch (overall) {
		case 'fail': return localize('glyphcodeHome.verdictFail', "verdict: fail");
		case 'error': return localize('glyphcodeHome.verdictError', "verdict: error");
		default:
			// A 'pass' is only an UNQUALIFIED pass through the blue door; otherwise it is a claim.
			return tone === 'blue'
				? localize('glyphcodeHome.verdictPass', "verdict: pass")
				: localize('glyphcodeHome.verdictPassUnverified', "verdict: pass (unverified)");
	}
}
