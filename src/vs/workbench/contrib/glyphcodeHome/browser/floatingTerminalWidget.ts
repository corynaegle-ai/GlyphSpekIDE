...
/*
 * GLYPHCODE FLOATING GOVERNED TERMINAL — the cursor-anchored IContentWidget (⌃⌘K, Option A).
 *
...
export class FloatingTerminalWidget extends Disposable implements IContentWidget {
	...
	constructor(
		...
		this.domNode.classList.add('glyphcode-floating-terminal', 'governed');
		this.domNode.setAttribute('role', 'group');
		this.domNode.setAttribute('aria-label', localize('glyphcode.floatingTerminal.aria', "GlyphCode governed terminal"));

		// Header: shield badge + honest posture copy (F3 — never trusted/verified/sandboxed).
		const header = document.createElement('div');
		header.classList.add('glyphcode-floating-terminal-header');
		const shield = document.createElement('span');
		shield.classList.add('glyphcode-floating-terminal-shield', ...ThemeIcon.asClassNameArray(Codicon.shield));
		const posture = document.createElement('span');
		posture.classList.add('glyphcode-floating-terminal-posture');
		posture.textContent = localize('glyphcode.floatingTerminal.posture', "governed-unsandboxed (soft) — egress traced");
		...
		describe.classList.add('glyphcode-floating-terminal-describe');
		describe.textContent = localize('glyphcode.floatingTerminal.describe', "Describe a command");
		describe.setAttribute('aria-label', localize('glyphcode.floatingTerminal.describe.aria', "Describe a command for GlyphCode to generate, pre-typed into the governed terminal"));
		...
		close.classList.add('glyphcode-floating-terminal-close', ...ThemeIcon.asClassNameArray(Codicon.close));
		close.setAttribute('aria-label', localize('glyphcode.floatingTerminal.close.aria', "Close Governed Terminal"));
		close.title = localize('glyphcode.floatingTerminal.close.title', "Close (Esc)");
		const onClose = () => this.onDismiss();
		close.addEventListener('click', onClose);
		this._register(toDisposable(() => close.removeEventListener('click', onClose)));
		header.appendChild(close);

		this.terminalHost.classList.add('glyphcode-floating-terminal-host');
		...
		grip.classList.add('glyphcode-floating-terminal-resize');
	...
	markDegraded(): void {
		...
		const posture = this.domNode.querySelector('.glyphcode-floating-terminal-posture');
		if (posture) {
			posture.textContent = localize('glyphcode.floatingTerminal.degraded', "governance lost — session degraded, finalizing");
		}
	}

	getId(): string {
		return 'glyphcode.floatingTerminal';
	}
 ...
 * (glyphcode.governedTerminal.startSession / .stopSession / .registerOwned /
...
export class FloatingTerminalController extends Disposable {
	 ...
	 * glyphcode.floatingTerminal.governanceLost(runId) for OUR runId. It feeds the pure
	...
	async open(): Promise<void> {
		...
		const start = await this.commandService.executeCommand<PureGovernedStartResult>('glyphcode.governedTerminal.startSession');
		...
		const deps: FloatingTerminalControllerDeps = {
			...
			stopSession: (runId: string) =>
				Promise.resolve(this.commandService.executeCommand('glyphcode.governedTerminal.stopSession', runId)),
			registerOwned: (runId: string) => {
				this.liveRunId = runId; // remember so the contribution can route governanceLost(runId).
				void this.commandService.executeCommand('glyphcode.governedTerminal.registerOwned', runId);
			},
			connectWidget: async () => {
			...
			showError: (message: string) => {
				void this.commandService.executeCommand('glyphcode.floatingTerminal.showError', message);
			},
		};
	...
	private showDegradedThenTeardown(widget: FloatingTerminalWidget): void {
		...
		void this.commandService.executeCommand('glyphcode.floatingTerminal.showDegradedNotice');
	}
	...
	async describeCommand(): Promise<void> {
		...
		const instruction = await this.quickInputService.input({
			prompt: localize('glyphcode.floatingTerminal.describePrompt', "Describe the command for GlyphCode to generate — it will be pre-typed into the governed terminal for you to review, then run."),
			placeHolder: localize('glyphcode.floatingTerminal.describePlaceholder', "e.g. list every file changed since main"),
		});
		if (instruction === undefined || instruction.trim().length === 0) {
			this.terminal.focus(true);
			return; // dismissed / empty — honest no-op (D5).
		}
		const cmd = await this.commandService.executeCommand<string | undefined>('glyphcode.governedTerminal.generateCommand', instruction);
		if (cmd && cmd.length > 0) {
