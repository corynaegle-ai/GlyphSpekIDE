...
/*
 * GlyphCode AGENT-RUN PROVIDER — the desktop (native Code-OSS workbench) side of the
 * Agent View seam (docs/glyphcode-agent-view-design.md §2). This is Part B of Slice 1.
 *
 ...
 *     `glyphcode.runs.snapshot` (Part A contract), which returns an AgentRunSnapshot.
 *   - It polls `glyphcode.runs.revision` (a monotonic number that advances on every
 ...
 * (src/vs/workbench/browser/parts/media/glyphcodeAuthorityHalo.css §"assurance → ring
 ...
 * `--gsh-*` trust-axis tokens in glyphcodeHome.css; the colorVar field names the matching
...
const SNAPSHOT_COMMAND_ID = 'glyphcode.runs.snapshot';

/**
 * The governed-run command (extension: "GlyphCode: Build This (Governed Run)"). This is the
 ...
 * seam. Kept in lockstep with BUILD_COMMAND_ID in glyphcodeHome.ts.
 */
...
const REVISION_COMMAND_ID = 'glyphcode.runs.revision';
 ...
 */
const DETAIL_COMMAND_ID = 'glyphcode.runs.detail';
...
/**
 * Narrow an unknown `glyphcode.runs.detail` result to AgentRunDetail WITHOUT an `any` cast.
...
export interface IAgentRunProvider extends IDisposable {
	...
	/**
	 * Fetch one run's EVIDENCE detail (Slice 2) via the read-only `glyphcode.runs.detail`
	 ...
	 * Backed by the existing governed-run command `glyphcode.promoteChatToBuild` (the SAME
	...
	/**
	 * True when the snapshot command is unregistered (the GlyphCode extension is not active
...
/**
 * The desktop provider. Polls `glyphcode.runs.revision`; on an advance, re-fetches
 * `glyphcode.runs.snapshot`, validates it, and fires onDidChangeRuns. Pure of DOM.
 */
export class DesktopAgentRunProvider extends Disposable implements IAgentRunProvider {
	...
	/**
	 * Fetch one run's evidence detail. Consumes the Part A `glyphcode.runs.detail` command,
	...
	/**
	 * Start a governed run (Slice 3). Dispatches the existing `glyphcode.promoteChatToBuild`
