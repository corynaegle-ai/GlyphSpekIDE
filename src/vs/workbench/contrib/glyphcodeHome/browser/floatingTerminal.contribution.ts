...
/*
 * GLYPHCODE FLOATING GOVERNED TERMINAL — the ⌃⌘K workbench action + per-window single-
...
const OPEN_FLOATING_TERMINAL_COMMAND_ID = 'glyphcode.openFloatingGovernedTerminal';
...
class FloatingTerminalManager extends Disposable implements IWorkbenchContribution {
	...
	static readonly ID = 'workbench.contrib.glyphcodeFloatingTerminal';
	...
	constructor(
		...
		// glyphcode.governedTerminal.generateCommand directly for the NL fold-in (D4a), so
		// no fork-side generate shim is needed.
		this._register(CommandsRegistry.registerCommand('glyphcode.floatingTerminal.showError', (_accessor, message?: string) => {
			this.notificationService.error(message
				? localize('glyphcode.floatingTerminal.error', "GlyphCode: the governed terminal could not be opened — {0}", message)
				: localize('glyphcode.floatingTerminal.errorGeneric', "GlyphCode: the governed terminal could not be opened."));
		}));
		...
		this._register(CommandsRegistry.registerCommand('glyphcode.floatingTerminal.governanceLost', (_accessor, runId?: string) => {
		...
		this._register(CommandsRegistry.registerCommand('glyphcode.floatingTerminal.showDegradedNotice', () => {
			this.notificationService.warn(localize('glyphcode.floatingTerminal.degradedNotice',
				"GlyphCode: governed terminal governance was lost mid-session — the session is degraded and is being finalized."));
		}));

		// D4a (unify target) — the palette NL-generate entry. The EXTENSION's
		// glyphcode.terminalGenerateCommand delegates here so the palette "describe a command"
		...
		this._register(CommandsRegistry.registerCommand('glyphcode.floatingTerminal.describeCommand', () => this.describeCommand()));
	 ...
	 * launches). This is what the extension's `glyphcode.terminalGenerateCommand` palette
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('glyphcode.openFloatingGovernedTerminal', 'Open Floating Governed Terminal at Cursor'),
			...
			metadata: {
				description: localize2('glyphcode.openFloatingGovernedTerminal.desc', 'Open a real PTY-backed GOVERNED terminal floating at the editor cursor (governed-unsandboxed, egress traced; refuses rather than degrades).'),
			},
		});
	}
