...
export class GettingStartedPage extends EditorPane {
	...
	private async buildCategoriesSlide(preserveFocus?: boolean) {
		...
		// GlyphStudio: brand hero. Show the GlyphStudio G-mark symbol (bracket "GS" mark only, no
		...
		const header = $('.header.glyphstudio-hero', {},
			$('.glyphstudio-logo', { 'role': 'img', 'aria-label': productName }),
			$('h1.product-name.caption', {}, ...titleChildren),
			$('p.subtitle.description', {}, localize({ key: 'gettingStarted.trustInfrastructure', comment: ['Shown as subtitle on the Welcome page. GlyphStudio product tagline.'] }, "Trust infrastructure for autonomous coding"))
		);
