...
/*
 * GLYPHSPEK FLOATING GOVERNED TERMINAL — the ⌃⌘K workbench action + per-window single-
...
const OPEN_FLOATING_TERMINAL_COMMAND_ID = 'glyphspek.openFloatingGovernedTerminal';
...
class FloatingTerminalManager extends Disposable implements IWorkbenchContribution {
	...
	static readonly ID = 'workbench.contrib.glyphspekFloatingTerminal';
	...
	constructor(
		...
		// glyphspek.governedTerminal.generateCommand directly for the NL fold-in (D4a), so
		// no fork-side generate shim is needed.
		this._register(CommandsRegistry.registerCommand('glyphspek.floatingTerminal.showError', (_accessor, message?: string) => {
			this.notificationService.error(message
				? localize('glyphspek.floatingTerminal.error', "GlyphSpek: the governed terminal could not be opened — {0}", message)
				: localize('glyphspek.floatingTerminal.errorGeneric', "GlyphSpek: the governed terminal could not be opened."));
		}));
		...
		this._register(CommandsRegistry.registerCommand('glyphspek.floatingTerminal.governanceLost', (_accessor, runId?: string) => {
		...
		this._register(CommandsRegistry.registerCommand('glyphspek.floatingTerminal.showDegradedNotice', () => {
			this.notificationService.warn(localize('glyphspek.floatingTerminal.degradedNotice',
				"GlyphSpek: governed terminal governance was lost mid-session — the session is degraded and is being finalized."));
		}));

		// D4a (unify target) — the palette NL-generate entry. The EXTENSION's
		// glyphspek.terminalGenerateCommand delegates here so the palette "describe a command"
		...
		this._register(CommandsRegistry.registerCommand('glyphspek.floatingTerminal.describeCommand', () => this.describeCommand()));
	 ...
	 * launches). This is what the extension's `glyphspek.terminalGenerateCommand` palette
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('glyphspek.openFloatingGovernedTerminal', 'Open Floating Governed Terminal at Cursor'),
			...
			metadata: {
				description: localize2('glyphspek.openFloatingGovernedTerminal.desc', 'Open a real PTY-backed GOVERNED terminal floating at the editor cursor (governed-unsandboxed, egress traced; refuses rather than degrades).'),
			},
		});
	}
