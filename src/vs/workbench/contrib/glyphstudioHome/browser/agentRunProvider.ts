...
/*
 * GlyphStudio AGENT-RUN PROVIDER — the desktop (native Code-OSS workbench) side of the
 * Agent View seam (docs/glyphstudio-agent-view-design.md §2). This is Part B of Slice 1.
 *
 ...
 *     `glyphstudio.runs.snapshot` (Part A contract), which returns an AgentRunSnapshot.
 *   - It polls `glyphstudio.runs.revision` (a monotonic number that advances on every
 ...
 * (src/vs/workbench/browser/parts/media/glyphstudioAuthorityHalo.css §"assurance → ring
 ...
 * `--gsh-*` trust-axis tokens in glyphstudioHome.css; the colorVar field names the matching
...
const SNAPSHOT_COMMAND_ID = 'glyphstudio.runs.snapshot';

/**
 * The governed-run command (extension: "GlyphStudio: Build This (Governed Run)"). This is the
 ...
 * seam. Kept in lockstep with BUILD_COMMAND_ID in glyphstudioHome.ts.
 */
...
const REVISION_COMMAND_ID = 'glyphstudio.runs.revision';
 ...
 */
const DETAIL_COMMAND_ID = 'glyphstudio.runs.detail';
...
/**
 * Narrow an unknown `glyphstudio.runs.detail` result to AgentRunDetail WITHOUT an `any` cast.
...
export interface IAgentRunProvider extends IDisposable {
	...
	/**
	 * Fetch one run's EVIDENCE detail (Slice 2) via the read-only `glyphstudio.runs.detail`
	 ...
	 * Backed by the existing governed-run command `glyphstudio.promoteChatToBuild` (the SAME
	...
	/**
	 * True when the snapshot command is unregistered (the GlyphStudio extension is not active
...
/**
 * The desktop provider. Polls `glyphstudio.runs.revision`; on an advance, re-fetches
 * `glyphstudio.runs.snapshot`, validates it, and fires onDidChangeRuns. Pure of DOM.
 */
export class DesktopAgentRunProvider extends Disposable implements IAgentRunProvider {
	...
	/**
	 * Fetch one run's evidence detail. Consumes the Part A `glyphstudio.runs.detail` command,
	...
	/**
	 * Start a governed run (Slice 3). Dispatches the existing `glyphstudio.promoteChatToBuild`
