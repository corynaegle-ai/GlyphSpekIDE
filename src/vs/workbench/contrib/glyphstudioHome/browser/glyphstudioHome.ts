...
import './media/glyphstudioHome.css';
import './media/floatingTerminal.css';
...
import { GlyphspekHomeInput } from './glyphstudioHomeInput.js';
import { createGlyphspekHomeIconSprite } from './glyphstudioHomeIcons.js';
import {
 ...
 * the GlyphStudio extension is not active (its commands unregistered), the row renders visibly
 * disabled with a "needs the GlyphStudio extension" hint rather than a live-looking no-op.
 */
...
/** The primary governed-run command (extension: "GlyphStudio: Build This (Governed Run)"). */
const BUILD_COMMAND_ID = 'glyphstudio.promoteChatToBuild';

/**
 * The governance command list. Each id is a command CONTRIBUTED by the GlyphStudio extension
...
const HOME_COMMANDS: readonly IHomeCommand[] = [
	{
		commandId: 'glyphstudio.openTrustPanel',
		label: localize('glyphstudioHome.openTrustPanel', "Open Trust Panel"),
		description: localize('glyphstudioHome.openTrustPanel.desc', "Evidence, not transcript — claims vs. signed verdict."),
		icon: 'gs-verifier'
	},
	{
		commandId: 'glyphstudio.runs.openTrustPanel',
		label: localize('glyphstudioHome.openVerifier', "Open Independent Verifier"),
		description: localize('glyphstudioHome.openVerifier.desc', "Re-runs declared checks in a clean trust domain and signs a verdict."),
		icon: 'gs-broker'
	},
	{
		commandId: 'workbench.view.extension.glyphstudio-egress',
		label: localize('glyphstudioHome.openEgress', "Open Egress / Network"),
		description: localize('glyphstudioHome.openEgress.desc', "Observe-and-trace egress (metadata-only) for governed runs — allowed-this-run is recorded, not blocked."),
		icon: 'gs-network'
	},
	{
		commandId: 'workbench.view.extension.glyphstudio-trace',
		label: localize('glyphstudioHome.openTrace', "Open Trace / Provenance"),
		description: localize('glyphstudioHome.openTrace.desc', "The hash-chained, tamper-evident event chain for the focused run."),
		icon: 'gs-trace'
	},
	{
		commandId: 'workbench.action.findInFiles',
		label: localize('glyphstudioHome.search', "Search"),
		description: localize('glyphstudioHome.search.desc', "Find across the workspace."),
		icon: 'gs-search'
	}
];
 ...
 *
 * Renders the blended-workbench aesthetic (floating frame, halo palette, GlyphStudio wordmark)
 * as a LIVE, HONEST surface: the primary input starts a REAL governed run via the extension's
 * `glyphstudio.promoteChatToBuild` command (typed task → build intent arg); each governance row
...
export class GlyphspekHomePage extends EditorPane {
	...
	static readonly ID = 'glyphstudioHomePage';
	...
	protected createEditor(parent: HTMLElement): void {
		this.container = $('.glyphstudio-home', {
			role: 'document',
			tabindex: 0,
			'aria-label': localize('glyphstudioHome.aria', "GlyphStudio Home — start a governed run and reach the governance surfaces.")
		});
		...
		try {
			...
			console.error('[GlyphStudio Home] icon sprite injection failed; rendering without brand icons', err);
		}
		append(parent, this.container);

		// Re-render the command rows when the GlyphStudio extension (de)registers its commands,
	...
	private renderBody(): void {
		...
		const scroll = append(this.container, $('.glyphstudio-home-scroll'));
		const inner = append(scroll, $('.glyphstudio-home-inner'));
	...
	private renderHeader(parent: HTMLElement): void {
		...
		title.textContent = 'GlyphStudio';
		const tagline = append(header, $('.gsh-tagline'));
		tagline.textContent = localize('glyphstudioHome.tagline', "Agent-safe engineering workstation — more autonomy, less blind trust.");
	}
	...
	private renderDraftComposer(parent: HTMLElement): void {
		...
		headText.textContent = localize('glyphstudioHome.startTitle', "Start a governed run");

		const sub = append(card, $('.gsh-card-sub'));
		sub.textContent = localize('glyphstudioHome.startSub', "Describe a task. The governed agent acts inside the supervisor envelope — it WILL edit files and run commands, every step brokered and traced.");

		const inputRow = append(card, $('.gsh-input-row'));
		const textarea = append(inputRow, $('textarea.gsh-task-input')) as HTMLTextAreaElement;
		textarea.placeholder = localize('glyphstudioHome.startPlaceholder', "e.g. \"add input validation to the signup form and a test for it\"");
		textarea.rows = 3;
		textarea.setAttribute('aria-label', localize('glyphstudioHome.startAria', "Describe the task for the governed agent to build"));
		...
		buttonLabel.textContent = localize('glyphstudioHome.startButton', "Start governed run");
		...
		if (buildAvailable) {
			...
			hintText.textContent = localize('glyphstudioHome.startHintLive', "Submits to the governed build (you approve authority up front). Press Enter to start, Shift+Enter for a newline.");
		} else {
			append(hint, this.icon('gs-broker', 'gsh-hint-icon gsh-hint-muted'));
			const hintText = append(hint, $('span.gsh-hint-muted'));
			hintText.textContent = localize('glyphstudioHome.startHintUnavailable', "The GlyphStudio extension is not active in this window, so governed runs can't start yet.");
		}
	}
	...
	private renderSelectedRunComposer(parent: HTMLElement): void {
		...
		append(head, this.icon('gs-run', 'gsh-head-icon'));
		const headText = append(head, $('span'));
		headText.textContent = localize('glyphstudioHome.composerSelectedTitle', "Selected run");
		...
		if (typeof intentText === 'string' && intentText.length > 0) {
			...
			intentBox.textContent = detail
				? localize('glyphstudioHome.composerNoIntent', "untitled run — no intent text was recorded.")
				: localize('glyphstudioHome.composerIntentLoading', "Loading the run's intent…");
		}
		...
		statusLabel.textContent = localize('glyphstudioHome.composerStatusLabel', "status");
		const statusValue = append(statusRow, $('span.gsh-composer-status-value'));
		if (detail) {
			statusValue.textContent = detail.closed
				? localize('glyphstudioHome.composerStatusClosed', "{0} (closed)", detail.status)
				: detail.status;
		} else {
			statusValue.textContent = localize('glyphstudioHome.composerStatusLoading', "loading…");
		}
		...
		append(hint, this.icon('gs-broker', 'gsh-hint-icon'));
		const hintText = append(hint, $('span'));
		hintText.textContent = localize('glyphstudioHome.composerSelectedHint', "Read-only — this run is already governed. Its evidence is on the right. Clear the selection to start a new run.");

		const back = append(card, $('button.gsh-composer-back')) as HTMLButtonElement;
		back.textContent = localize('glyphstudioHome.composerBack', "Start a new run instead");
	...
	private renderCommands(parent: HTMLElement): void {
		...
		sectionHead.textContent = localize('glyphstudioHome.commandsTitle', "Governance surfaces");
	...
	private renderCommandRow(parent: HTMLElement, cmd: IHomeCommand): void {
		...
		desc.textContent = available
			? cmd.description
			: localize('glyphstudioHome.rowUnavailable', "Needs the GlyphStudio extension active in this window.");
	...
	private renderRunsList(parent: HTMLElement): void {
		...
		const sectionHead = append(section, $('.gsh-section-head'));
		sectionHead.textContent = localize('glyphstudioHome.runsTitle', "Runs");
	...
	private renderRunsEmpty(parent: HTMLElement): void {
		...
		if (!this.runProvider.isExtensionActive()) {
			emptyTitle.textContent = localize('glyphstudioHome.runsExtensionInactive', "GlyphStudio extension not active");
			emptySub.textContent = localize('glyphstudioHome.runsExtensionInactiveSub', "Runs are owned by the GlyphStudio extension. Activate it in this window to see governed runs here.");
		} else {
			emptyTitle.textContent = localize('glyphstudioHome.runsEmpty', "No runs yet — start one in the center.");
			emptySub.textContent = localize('glyphstudioHome.runsEmptySub', "Governed runs you start appear here; selecting one shows its evidence on the right.");
		}
	}
	...
	private renderRunRow(parent: HTMLElement, run: AgentRun): void {
		...
		if (run.closed) {
			...
			closedEl.textContent = localize('glyphstudioHome.runClosed', "closed");
		}
	...
	private statePillLabel(state: RunCardState): string {
		switch (state) {
			case 'verified': return localize('glyphstudioHome.pillVerified', "verified");
			case 'blocked': return localize('glyphstudioHome.pillBlocked', "blocked");
			default: return localize('glyphstudioHome.pillActing', "acting");
		}
	}
	 ...
	 * `src/vs/workbench/browser/parts/glyphstudioAuthorityHalo.ts` mirrors the
	 * `glyphstudio.authority` CONTEXT-KEY (which the EXTENSION sets for the FOCUSED run) onto a
	 * `data-glyphstudio-authority` attribute on `.monaco-workbench`, and the companion CSS paints
	...
	private applyAuthorityTint(): void {
		...
		if (level) {
			...
			panes.title = localize('glyphstudioHome.authorityTitle', "Authority halo: {0}", authorityLabel(level));
		} else {
	 ...
	 * run's AgentRunDetail (fetched via the provider's `glyphstudio.runs.detail` consumer).
	...
	private renderEvidencePane(parent: HTMLElement): void {
		const section = append(parent, $('.gsh-section'));
		const sectionHead = append(section, $('.gsh-section-head'));
		sectionHead.textContent = localize('glyphstudioHome.evidenceTitle', "Evidence");
	...
	private renderEvidenceBody(): void {
		...
		if (!this.selectedRunId) {
			this.renderEvidencePlaceholder(
				body,
				localize('glyphstudioHome.evidenceNone', "Select a run to see its evidence."),
				localize('glyphstudioHome.evidenceNoneSub', "Its posture, changes, and signed verdict appear here.")
			);
			return;
		}
		...
		if (!detail) {
			...
			if (!this.runProvider.isExtensionActive()) {
				this.renderEvidencePlaceholder(
					body,
					localize('glyphstudioHome.evidenceInactive', "GlyphStudio extension not active"),
					localize('glyphstudioHome.evidenceInactiveSub', "Run evidence is owned by the GlyphStudio extension. Activate it in this window to load it here.")
				);
			} else {
				this.renderEvidencePlaceholder(
					body,
					localize('glyphstudioHome.evidenceLoading', "Loading evidence…"),
					localize('glyphstudioHome.evidenceLoadingSub', "Fetching the selected run's posture, changes, and verdict.")
				);
			}
			return;
		}
	...
	private renderEvidenceHeader(parent: HTMLElement, detail: AgentRunDetail): void {
		...
		statusEl.textContent = detail.closed
			? localize('glyphstudioHome.evidenceStatusClosed', "{0} (closed)", detail.status)
			: detail.status;
	...
	private renderPostureSection(parent: HTMLElement, detail: AgentRunDetail): void {
		const body = this.openEvidenceSection(parent, 'gs-sandbox', localize('glyphstudioHome.evPosture', "Posture"));
	...
	private renderChangesSection(parent: HTMLElement, detail: AgentRunDetail): void {
		const body = this.openEvidenceSection(parent, 'gs-policy', localize('glyphstudioHome.evChanges', "Changes"));
		if (detail.changedFiles.length === 0 && detail.diff.trim().length === 0) {
			const none = append(body, $('.gsh-ev-muted'));
			none.textContent = localize('glyphstudioHome.evNoChanges', "No changes recorded for this run yet.");
			return;
		}
		...
		if (detail.diff.trim().length > 0) {
			...
			caption.textContent = localize('glyphstudioHome.evDiffCaption', "Review surface — the unified diff of the run's changes (not a sandbox).");
		}
	}
	...
	private renderVerdictSection(parent: HTMLElement, detail: AgentRunDetail): void {
		const body = this.openEvidenceSection(parent, 'gs-verifier', localize('glyphstudioHome.evVerdict', "Verdict"));
		const verdict = detail.verdict;
		if (!verdict) {
			// Honest "no verdict yet" — never a fabricated PASS. Distinct from "select a run".
			const none = append(body, $('.gsh-ev-muted'));
			none.textContent = localize('glyphstudioHome.evNoVerdict', "No evidence yet — this run has no signed verifier verdict.");
			return;
		}
		...
		assuranceEl.textContent = verdict.assurance === 'full'
			? localize('glyphstudioHome.evAssuranceFull', "assurance: full")
			: localize('glyphstudioHome.evAssuranceDegraded', "assurance: degraded");

		// --- signature state (presence ≠ trust) ---
		const sigEl = append(verdictCard, $('.gsh-ev-signature'));
		if (blueEligible) {
			sigEl.classList.add('gsh-ev-sig-verified');
			sigEl.textContent = localize('glyphstudioHome.evSigVerified', "signature verified · product-trusted");
			...
			sigEl.textContent = detail.verified
				...
				? localize('glyphstudioHome.evSigSoftCap', "signature present · capped by soft posture (not product-trusted)")
				: localize('glyphstudioHome.evSigUnverified', "signature present · not verified");
		} else {
			sigEl.classList.add('gsh-ev-sig-none');
			sigEl.textContent = localize('glyphstudioHome.evSigNone', "no signature");
		}
		...
		if (verdict.verifierIsolation || verdict.verifyCommandSource) {
			...
			if (verdict.verifierIsolation) {
				...
				iso.textContent = verdict.verifierIsolation === 'independent-sandboxed'
					? localize('glyphstudioHome.evIsoIndependent', "verifier: independent-sandboxed")
					: localize('glyphstudioHome.evIsoInline', "verifier: inline-unsandboxed");
			}
			if (verdict.verifyCommandSource) {
				...
				src.textContent = localize('glyphstudioHome.evCmdSource', "checks: {0}", verdict.verifyCommandSource);
			}
		}
		...
		if (verdict.traceRootHash) {
			...
			rootLabel.textContent = localize('glyphstudioHome.evTraceRoot', "trace root");
	...
	private renderCheck(parent: HTMLElement, check: AgentRunCheck): void {
		...
		status.textContent = check.status === 'skipped'
			? localize('glyphstudioHome.evCheckSkipped', "skipped")
			: check.status;
	}
...
function postureLabel(posture: CreationTrustPosture): string {
	switch (posture) {
		case 'trusted': return localize('glyphstudioHome.postureTrusted', "Trusted (pending gate)");
		case 'sandboxed-soft-egress': return localize('glyphstudioHome.postureSandboxedSoft', "Sandboxed (soft egress)");
		case 'governed-unsandboxed': return localize('glyphstudioHome.postureGovernedUnsandboxed', "Governed (soft) — unsandboxed");
		case 'untrusted': return localize('glyphstudioHome.postureUntrusted', "Untrusted");
		case 'refused': return localize('glyphstudioHome.postureRefused', "Refused");
		default: return localize('glyphstudioHome.postureUnknown', "Starting…");
	}
}
...
function postureFraming(posture: CreationTrustPosture): string {
	switch (posture) {
		case 'trusted':
			return localize('glyphstudioHome.postureFramingTrusted', "Hard isolation + hard egress reported. Eligible for a product-trusted verdict ONLY after the signature gate passes.");
		case 'sandboxed-soft-egress':
			return localize('glyphstudioHome.postureFramingSandboxedSoft', "SOFT — not contained. Isolation runtime, but the egress boundary is soft. Never product-trusted.");
		case 'governed-unsandboxed':
			return localize('glyphstudioHome.postureFramingGovernedUnsandboxed', "SOFT — not contained. Governed + traced via the metadata-only egress proxy, but unsandboxed. Never product-trusted.");
		case 'untrusted':
			return localize('glyphstudioHome.postureFramingUntrusted', "No trusted runtime could be established. Cannot be product-trusted.");
		case 'refused':
			return localize('glyphstudioHome.postureFramingRefused', "The supervisor refused to create the run. No trusted run was produced.");
		default:
			return localize('glyphstudioHome.postureFramingUnknown', "Trust posture not settled yet (no RunOpened observed).");
	}
}
...
function verdictOverallLabel(overall: 'pass' | 'fail' | 'error', tone: 'blue' | 'amber' | 'red'): string {
	switch (overall) {
		case 'fail': return localize('glyphstudioHome.verdictFail', "verdict: fail");
		case 'error': return localize('glyphstudioHome.verdictError', "verdict: error");
		default:
			// A 'pass' is only an UNQUALIFIED pass through the blue door; otherwise it is a claim.
			return tone === 'blue'
				? localize('glyphstudioHome.verdictPass', "verdict: pass")
				: localize('glyphstudioHome.verdictPassUnverified', "verdict: pass (unverified)");
	}
}
