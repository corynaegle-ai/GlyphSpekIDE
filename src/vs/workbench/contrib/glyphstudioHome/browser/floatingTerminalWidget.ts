...
/*
 * GLYPHSTUDIO FLOATING GOVERNED TERMINAL — the cursor-anchored IContentWidget (⌃⌘K, Option A).
 *
...
export class FloatingTerminalWidget extends Disposable implements IContentWidget {
	...
	constructor(
		...
		this.domNode.classList.add('glyphstudio-floating-terminal', 'governed');
		this.domNode.setAttribute('role', 'group');
		this.domNode.setAttribute('aria-label', localize('glyphstudio.floatingTerminal.aria', "GlyphStudio governed terminal"));

		// Header: shield badge + honest posture copy (F3 — never trusted/verified/sandboxed).
		const header = document.createElement('div');
		header.classList.add('glyphstudio-floating-terminal-header');
		const shield = document.createElement('span');
		shield.classList.add('glyphstudio-floating-terminal-shield', ...ThemeIcon.asClassNameArray(Codicon.shield));
		const posture = document.createElement('span');
		posture.classList.add('glyphstudio-floating-terminal-posture');
		posture.textContent = localize('glyphstudio.floatingTerminal.posture', "governed-unsandboxed (soft) — egress traced");
		...
		describe.classList.add('glyphstudio-floating-terminal-describe');
		describe.textContent = localize('glyphstudio.floatingTerminal.describe', "Describe a command");
		describe.setAttribute('aria-label', localize('glyphstudio.floatingTerminal.describe.aria', "Describe a command for GlyphStudio to generate, pre-typed into the governed terminal"));
		...
		close.classList.add('glyphstudio-floating-terminal-close', ...ThemeIcon.asClassNameArray(Codicon.close));
		close.setAttribute('aria-label', localize('glyphstudio.floatingTerminal.close.aria', "Close Governed Terminal"));
		close.title = localize('glyphstudio.floatingTerminal.close.title', "Close (Esc)");
		const onClose = () => this.onDismiss();
		close.addEventListener('click', onClose);
		this._register(toDisposable(() => close.removeEventListener('click', onClose)));
		header.appendChild(close);

		this.terminalHost.classList.add('glyphstudio-floating-terminal-host');
		...
		grip.classList.add('glyphstudio-floating-terminal-resize');
	...
	markDegraded(): void {
		...
		const posture = this.domNode.querySelector('.glyphstudio-floating-terminal-posture');
		if (posture) {
			posture.textContent = localize('glyphstudio.floatingTerminal.degraded', "governance lost — session degraded, finalizing");
		}
	}

	getId(): string {
		return 'glyphstudio.floatingTerminal';
	}
 ...
 * (glyphstudio.governedTerminal.startSession / .stopSession / .registerOwned /
...
export class FloatingTerminalController extends Disposable {
	 ...
	 * glyphstudio.floatingTerminal.governanceLost(runId) for OUR runId. It feeds the pure
	...
	async open(): Promise<void> {
		...
		const start = await this.commandService.executeCommand<PureGovernedStartResult>('glyphstudio.governedTerminal.startSession');
		...
		const deps: FloatingTerminalControllerDeps = {
			...
			stopSession: (runId: string) =>
				Promise.resolve(this.commandService.executeCommand('glyphstudio.governedTerminal.stopSession', runId)),
			registerOwned: (runId: string) => {
				this.liveRunId = runId; // remember so the contribution can route governanceLost(runId).
				void this.commandService.executeCommand('glyphstudio.governedTerminal.registerOwned', runId);
			},
			connectWidget: async () => {
			...
			showError: (message: string) => {
				void this.commandService.executeCommand('glyphstudio.floatingTerminal.showError', message);
			},
		};
	...
	private showDegradedThenTeardown(widget: FloatingTerminalWidget): void {
		...
		void this.commandService.executeCommand('glyphstudio.floatingTerminal.showDegradedNotice');
	}
	...
	async describeCommand(): Promise<void> {
		...
		const instruction = await this.quickInputService.input({
			prompt: localize('glyphstudio.floatingTerminal.describePrompt', "Describe the command for GlyphStudio to generate — it will be pre-typed into the governed terminal for you to review, then run."),
			placeHolder: localize('glyphstudio.floatingTerminal.describePlaceholder', "e.g. list every file changed since main"),
		});
		if (instruction === undefined || instruction.trim().length === 0) {
			this.terminal.focus(true);
			return; // dismissed / empty — honest no-op (D5).
		}
		const cmd = await this.commandService.executeCommand<string | undefined>('glyphstudio.governedTerminal.generateCommand', instruction);
		if (cmd && cmd.length > 0) {
