...
import { GlyphspekHomePage } from './glyphstudioHome.js';
import { GlyphspekHomeInput, glyphstudioHomeInputTypeId } from './glyphstudioHomeInput.js';

/** Setting that controls whether GlyphStudio Home auto-opens when the editor area would be empty. */
const OPEN_ON_STARTUP_SETTING = 'glyphstudio.home.openOnStartup';

/** Command id for re-opening Home (palette: "GlyphStudio: Open Home"). */
const OPEN_HOME_COMMAND_ID = 'glyphstudio.home.open';
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
		...
		localize('glyphstudioHome', "GlyphStudio Home")
	),
	[
...
// ---- "GlyphStudio: Open Home" command. ----
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: OPEN_HOME_COMMAND_ID,
			title: localize2('glyphstudioHome.open', 'Open Home'),
			category: Categories.View,
			f1: true,
			// "Back to Home" chord. Verified unused: Cmd+K Cmd+H is the Output panel, so use
			// Cmd+K Cmd+G ("G" for GlyphStudio), which has no default binding on any platform.
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
				primary: KeyChord(KeyMod.CtrlCmd | KeyCode.KeyK, KeyMod.CtrlCmd | KeyCode.KeyG)
			},
			metadata: {
				description: localize2('glyphstudioHome.open.desc', 'Open the GlyphStudio Home surface — start a governed run and reach the governance surfaces.')
			}
		});
	}
...
MenuRegistry.appendMenuItem(MenuId.MenubarViewMenu, {
	...
	command: {
		id: OPEN_HOME_COMMAND_ID,
		title: localize({ key: 'miGlyphspekHome', comment: ['&& denotes a mnemonic'] }, "GlyphStudio &&Home")
	},
	order: 2
});
...
class GlyphspekHomeEditorResolverContribution extends Disposable implements IWorkbenchContribution {
	...
	static readonly ID = 'workbench.contrib.glyphstudioHomeEditorResolver';
	...
	constructor(
		...
		this._register(editorResolverService.registerEditor(
			`${GlyphspekHomeInput.RESOURCE.scheme}:/**`,
			{
				...
				label: localize('glyphstudioHome.displayName', "GlyphStudio Home"),
...
/**
 * Lands the IDE on GlyphStudio Home on every fresh app launch — the way an agent-home IDE opens to
 * its home rather than to last session's panes. Modeled on `StartupPageRunnerContribution`: waits
 * for `Restored`, then (behind `glyphstudio.home.openOnStartup`, default ON) opens Home as the
...
class GlyphspekHomeStartupContribution extends Disposable implements IWorkbenchContribution {
	...
	static readonly ID = 'workbench.contrib.glyphstudioHomeStartup';
	...
	private async run(): Promise<void> {
		...
		const homeAlreadyOpen = this.editorService.editors.some(e => e.typeId === glyphstudioHomeInputTypeId);
...
// ---- GlyphStudio Home configuration. ----
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	id: 'glyphstudio',
	order: 9,
	title: localize('glyphstudioHomeConfigurationTitle', "GlyphStudio"),
	type: 'object',
	properties: {
		[OPEN_ON_STARTUP_SETTING]: {
			...
			description: localize('glyphstudio.home.openOnStartup', "Open the GlyphStudio Home surface as the active tab on startup (the way an agent-home IDE lands on its home). Restored editors stay open behind it; a window reload keeps your current editor focused. Reopen any time with the \"GlyphStudio: Open Home\" command.")
		}
	}
});
