...
/**
 * GlyphSpek PATCH-001 (Sweep-19 Finding 2) — fail-closed bundled Sovereign policy source.
 *
 ...
 * `glyphspekSovereignPolicyFile`). When the bundled policy file is missing/unreadable/invalid
...
export class SovereignFilePolicyService extends FilePolicyService {
	...
	/** The policy whose value carries the GlyphSpek-curated extension allowlist. */
	...
	protected override onReadFailed(error: unknown, policies: Map<PolicyName, PolicyValue>): void {
		...
		this.logService.error(
			`[SovereignFilePolicyService] GlyphSpek Sovereign policy file could not be loaded ` +
