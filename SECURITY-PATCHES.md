# GlyphCode — Workbench Security Patch Ledger
...
> `build/glyphcode/sovereign-profile/README.md` for the Sovereign-profile decision that
...
- `product.json` — added one additive key `"glyphcodeSovereignPolicyFile": true` (the gate),
  and (separately, M1 cleanliness Task 2) emptied `webviewContentExternalBaseUrlTemplate`.
- `src/vs/base/common/product.ts` — declared the `glyphcodeSovereignPolicyFile?: boolean`
  field on `IProductConfiguration`.
- `src/vs/platform/environment/common/environmentService.ts` — added a memoized
  `glyphcodeSovereignPolicyFile: URI | undefined` getter (analogous to `policyFile`): returns
  `<appRoot>/policy.json` only when the product flag is set, overridable by
  `--glyphcode-policy-file <path>`. A stock/Developer build (flag absent) returns `undefined`
  and is wholly unaffected.
- `src/vs/platform/environment/node/argv.ts` + `common/argv.ts` — added the non-hidden
  `--glyphcode-policy-file <path>` string arg.
...
- `src/vs/code/electron-main/main.ts` — in the policy-source selection block, added a
  leading branch: when `glyphcodeSovereignPolicyFile` is set, build a `FilePolicyService` over
...
- `src/vs/platform/policy/test/common/multiPolicyService.test.ts` — **new** acceptance test
  (`suite('MultiPolicyService (GlyphCode PATCH-001)')`) covering acceptance #1 (standalone
...
**Bundled policy source of truth:** `build/glyphcode/sovereign-profile/sovereign-policy.json`
...
`glyphcodeSovereignPolicyFile` set but `<appRoot>/policy.json` absent/unreadable/invalid, the
...
- `src/vs/code/electron-main/main.ts` + `src/vs/code/node/cliProcessMain.ts` — the bundled
  ...
  inside the `if (environment…glyphcodeSovereignPolicyFile) { … }` branch (gated by the
...
`glyphcodeSovereignPolicyFile`, copies `build/glyphcode/sovereign-profile/sovereign-policy.json`
...
new suite `suite('SovereignFilePolicyService fail-closed (GlyphCode PATCH-001 / Sweep-19 F2)')`:
...
- `src/vs/code/electron-main/main.ts` — in the policy-source selection block
  (currently lines ~213-229): when a GlyphCode-bundled policy file is present (resolved from
  the app resources dir, or gated by a new `product.json` boolean e.g.
  `glyphcodeSovereignPolicyFile`), construct a `FilePolicyService` over that bundled file —
...
- `src/vs/code/node/cliProcessMain.ts` — mirror the same selection (lines ~181-193) so the
  CLI (`glyphcode --install-extension`, etc.) honors the same allowlist at install time.
- (If a flag is preferred over auto-detect) `src/vs/platform/environment/node/argv.ts` +
  ...
  `--glyphcode-policy-file <path>` arg resolving to a `URI`, analogous to the existing
...
`glyphcodeSovereignPolicyFile` flag approach is chosen, that single additive `product.json`
...
1. Build a Sovereign artifact on macOS (and Windows) with a bundled policy file containing
   `{ "AllowedExtensions": { "glyphcode": true, "*": false } }`, **no** MDM/registry policy
...
4. **Layering check:** with the bundled file allowing `glyphcode` + one curated id, set an
...
- **Rebase risk: LOW.** The policy-source selection block in `main.ts`/`cliProcessMain.ts` is
  ...
  GlyphCode branch as an added `else if` ahead of the `NullPolicyService` fallback to
  minimize conflict surface. Re-verify upstream hasn't renamed
  `LINUX_SYSTEM_POLICY_FILE_PATH`/`policyFile` on each rebase.

---

## PATCH-002 — Workbench chat-default resolver honors the first-party GlyphCode vendor
...
- **Required:** for the de-Copilot fork to answer in the stock "Build with Agent" chat panel.
  ...
  reaches the GlyphCode chat participant.
...
model whose `vendor === GLYPHCODE_DEFAULT_MODEL_VENDOR` (`'glyphcode'`, a named constant
...
vendor: if neither a Copilot nor a `glyphcode` chat-default exists it **fails closed**
...
- the embedded extension publishes exactly one language model marked `isDefault: true`,
  vendor `glyphcode` (`extension/src/chatParticipant.ts`,
...
- `product.json` grants the `defaultChatParticipant` + `chatProvider` API proposals to
  `glyphcode.glyphcode` (`extensionEnabledApiProposals`). The `chatProvider` grant is what
...
unavailable" — the panel refused to send and never reached the GlyphCode chat participant.
...
**GlyphCode-owned governed provider** as the implicit "Auto" default, never some other
vendor's model. A non-GlyphCode provider silently becoming the "Auto" default would route a
user's unattributed prompt through an unintended (and potentially un-governed) model with no
visible selection — so the resolver fails closed to a GlyphCode-only invariant.
...
- The resolver `getDefaultLanguageModel` lives in the **workbench extension host**
  ...
  model (and the GlyphCode extension does, with `isDefault: true`), but it cannot change
...
- The supervisor governs the runs/terminals/chat GlyphCode creates; it has no hook into the
...
- `src/vs/workbench/api/common/extHostLanguageModels.ts` — added the
  `GLYPHCODE_DEFAULT_MODEL_VENDOR = 'glyphcode'` named constant (with comment) and rewrote the
...
- *Non-workbench (recorded for context, not part of this `src/vs` diff):*
  ...
  vendor `glyphcode` model metadata); `product.json` `extensionEnabledApiProposals` granting
  `defaultChatParticipant` + `chatProvider` to `glyphcode.glyphcode`.

### Acceptance test

- `src/vs/workbench/api/test/common/extHostLanguageModels.test.ts` —
  `suite('ExtHostLanguageModels')`:
  1. **Happy path:** with only the `glyphcode` chat-default provider registered (the shipped
     config), `getDefaultLanguageModel` resolves the `glyphcode`/`glyphcode-codex-gateway`
     model (chat keeps working on "Auto").
  2. **Adversarial / order-independence:** a second non-Copilot `isDefault` provider (vendor
     `'other'`) registers **before** the `glyphcode` one; the resolver still resolves the
     `glyphcode` model, never `'other'` — registration order cannot hand "Auto" to a
     non-GlyphCode vendor.
  3. **Fail-closed:** with only the `'other'` provider registered (no `glyphcode`, no
     Copilot), `getDefaultLanguageModel` returns `undefined` (honest "Language model
     unavailable"), proving a non-GlyphCode default cannot win "Auto".
- `extension/test/chatParticipantAgent.test.mjs` — asserts the embedded extension registers
  its LM provider under `CHAT_MODEL_VENDOR` (`glyphcode`) and publishes the single model with
...
- **Rebase risk: LOW–MEDIUM.** `getDefaultLanguageModel` is small and stable upstream, but
  ...
  `vendor === GLYPHCODE_DEFAULT_MODEL_VENDOR`, then fail closed. Re-verify upstream hasn't
...
A workbench contribution (`GlyphCodeAuthorityHaloContribution`) observes the `glyphcode.authority`
**context-key** the GlyphCode first-party extension already sets on every assurance change
(`extension/src/extension.ts` `setContext('glyphcode.authority', <level>)`; values
`read|claimed|soft|verified|denied`, default `read`). It mirrors that value to a
`data-glyphcode-authority` attribute on the `.monaco-workbench` root (`IWorkbenchLayoutService.mainContainer`);
...
workbench (no inset), behind the setting `glyphcode.workbench.floatingFrame` (default ON,
...
`glyphcode.workbench.haloMotion` setting (contribution toggles a class): either disables the
...
- `src/vs/workbench/browser/parts/glyphcodeAuthorityHalo.ts` — **new** contribution
  (`GlyphCodeAuthorityHaloContribution`, registered `registerWorkbenchContribution2` @
  `WorkbenchPhase.AfterRestored`) + the two `glyphcode.workbench.*` settings registration.
- `src/vs/workbench/browser/parts/media/glyphcodeAuthorityHalo.css` — **new** the ring color/glow
  map, `0.5s` cross-fade, `glyphcodeDenyPulse`, floating-frame substrate, reduced-motion rules.
...
2. Drive a run's assurance via the extension (Trust Panel / demo run): the ring tracks
   `glyphcode.authority` — `claimed`→amber, `soft`→violet, `verified`→blue (only on a
...
4. With OS "reduce motion" on **or** `glyphcode.workbench.haloMotion: false`, no pulse / no
   cross-fade — the static assurance color remains.
5. Toggle `glyphcode.workbench.floatingFrame: false` — the substrate/rounded frame drops to the
...
- **Rollback:** delete `src/vs/workbench/browser/parts/glyphcodeAuthorityHalo.ts` +
  `…/media/glyphcodeAuthorityHalo.css` and remove the one import line in
  `workbench.common.main.ts`. The `data-glyphcode-authority` attribute and the two settings then
...
## PATCH-004 — GlyphCode Home landing surface (runs-first editor-area home)
...
Registers a **GlyphCode Home** editor (an `EditorInput` + `EditorPane`, native-DOM, modeled on
the stock Welcome/GettingStarted editor) that fills the editor area and **opens on startup** as
the landing tab, re-openable via the `GlyphCode: Open Home` command. It renders the
blended-workbench mockup as a live surface: a GlyphCode wordmark, a primary **"Start a governed
...
- **No dead buttons.** Every governance row gates on `CommandsRegistry.getCommand(id)` at render
  time. If the GlyphCode first-party extension is not active, the row renders **visibly disabled**
  with a "Needs the GlyphCode extension active" hint and live-upgrades to enabled via an
...
- **Real command, real arg.** The primary input invokes the actual governed-build entry command
  `glyphcode.promoteChatToBuild` (the same path as "Build This (Governed Run)"), passing the typed
...
- `GlyphspekHomeStartupContribution` (registered `AfterRestored`, mirroring stock
  `StartupPageRunnerContribution`) gates on `glyphcode.home.openOnStartup` (new setting, default
...
- `glyphcode.promoteChatToBuild` ← the "Start a governed run" input (primary).
- `glyphcode.openTrustPanel` → Open Trust Panel · `glyphcode.runs.openTrustPanel` → Open
  Independent Verifier · `workbench.view.extension.glyphcode-egress` → Egress / Network ·
  `workbench.view.extension.glyphcode-trace` → Trace / Provenance · `workbench.action.findInFiles`
  → Search (stock).
- New: `glyphcode.home.open` → "GlyphCode: Open Home" (palette + View category, `revealIfOpened`).
...
- `src/vs/workbench/contrib/glyphcodeHome/browser/glyphcodeHomeInput.ts` — **new** the
  `EditorInput` (resource-keyed, single-per-resource).
- `src/vs/workbench/contrib/glyphcodeHome/browser/glyphcodeHome.ts` — **new** the `EditorPane`
  (`GlyphspekHomePage`, native-DOM render).
- `src/vs/workbench/contrib/glyphcodeHome/browser/glyphcodeHomeIcons.ts` — **new** brand icon
  sprite, vendored verbatim from `docs/assets/glyphcode-icons.svg`.
- `src/vs/workbench/contrib/glyphcodeHome/browser/glyphcodeHome.contribution.ts` — **new**
  pane/input/serializer registration, the `glyphcode.home.open` command, the editor resolver, the
  startup auto-open runner, and the `glyphcode.home.openOnStartup` setting.
- `src/vs/workbench/contrib/glyphcodeHome/browser/media/glyphcodeHome.css` — **new** styling,
...
1. Launch the rebuilt app on a clean profile with no folder → **GlyphCode Home** opens as the
   landing editor (wordmark, "Start a governed run" input, governance rows, "No runs yet" empty
   state).
2. Type a task into the input and submit → `glyphcode.promoteChatToBuild` fires with that text as
   the build intent (the governed-build flow starts).
3. With the GlyphCode extension **inactive**, the Trust Panel / Verifier / Egress / Trace rows
   render **disabled** with the "needs the extension active" hint; on activation they enable live
   (no reload).
4. `GlyphCode: Open Home` from the palette re-opens Home (`revealIfOpened` — no duplicate tab).
5. Fresh launch with restored editors → Home opens as the **active** tab in front of them (they
   ...
   `glyphcode.home.openOnStartup: false` disables the startup-open entirely.
...
- **Rollback:** delete `src/vs/workbench/contrib/glyphcodeHome/` and remove the one import line in
  `workbench.common.main.ts`. The `glyphcode.home.open` command + `glyphcode.home.openOnStartup`
...
stubs** whose every Copilot entry point throws `"Copilot is not available in GlyphCode"` or
...
- **Stub packages (new):** `build/glyphcode/copilot-stubs/`
  - `github-copilot-sdk/` — `dist/index.js` (inert runtime) + the SDK's real `*.d.ts`
    ...
    `@glyphcode/copilot-sdk-stub` (NOT `@github/copilot-sdk`, so the deny gate cannot
    match it).
...
- `src/typings/copilot-api.d.ts` → renamed to `…d.ts.glyphcode-removed` (the stub's
...
3. `build/glyphcode/verify-no-copilot.sh` → **PASS** (exit 0); `find …GlyphCode.app -path
...
4. **Render / runtime:** app launches with no bootstrap exception; main log shows
   ...
   walkthrough renders, and **"GlyphCode: Open Trust Panel" opens and renders fully** (all
   sections: live runs, run trust state, actor plan & claims, verifier verdict, live trace
   timeline, changed files). Screenshots in `/tmp/glyphcode-decopilot/`.
...
GlyphCode" / auth-required error rather than crash. GlyphCode's own trust/agent layer is
unaffected.
...
`src/typings/copilot-api.d.ts`; delete `build/glyphcode/copilot-stubs/`.
...
- **New:** `build/glyphcode/verify-no-copilot.sh` — scans the WHOLE app bundle for Copilot
...
- **Chained:** `build/glyphcode/verify-bundled-extensions.sh` now runs `verify-no-copilot.sh`
...
- **Threat addressed:** false release signal. A "PASS" deny gate while GitHub Copilot code +
  SDK material sits in the app bundle masks a guardrail violation (GlyphCode ships no Copilot)
...
1. Remove or re-platform the Agent Host's Copilot path: either delete
   ...
   `@github/copilot-sdk` / `@vscode/copilot-api` runtime with a GlyphCode-owned brokered model
   client.
...
3. Rebuild and run `verify-bundled-extensions.sh` (Part 1): it must then PASS, and
   `find VSCode-darwin-arm64/GlyphCode.app -path '*node_modules/@github/copilot*'` must be empty.

### Rollback / rebase note

- **Part 1 rollback:** delete `build/glyphcode/verify-no-copilot.sh` and revert the chaining
  block in `verify-bundled-extensions.sh`. No app/runtime impact (gates are build-time only).
- **Rebase risk: LOW.** Both files are GlyphCode-only build tooling with no upstream
  counterpart.
...
- **Origin:** every code-review sweep flagged `verify-no-telemetry.sh` as **REVIEW/fail** on
  ...
  distinction between GlyphCode-introduced telemetry (must be zero) and dormant
...
GlyphCode-introduced telemetry leak would be lost in the noise of the same ~50 expected vendored
hits. The fix makes the gate's verdict **accurate** — PASS when (and only when) GlyphCode
introduces no telemetry and the sole hits are the inventoried vendored-dormant set; **FAIL** the
instant a telemetry string appears outside that inventory (GlyphCode-authored or a new vendored
...
Scanned the packaged app (`../VSCode-darwin-arm64/GlyphCode.app`). The HARD hits resolve to **29
unique files**; the gate's per-pattern total is **50** (files matching multiple patterns):

- **GlyphCode-INTRODUCED telemetry: ZERO.** The embedded `glyphcode-trust-panel` extension
  ...
  absent, gallery = `open-vsx.org`, `reportIssueUrl` = the GlyphCode repo.
- **VENDORED-DORMANT: 29 files**, inventoried in
  `build/glyphcode/vendored-telemetry-allowlist.json` (9 Microsoft built-in extensions + 11
  ...
  `build/glyphcode/telemetry-posture.md` for the verified-vs-assumed breakdown. **Assumed (not
...
- `build/glyphcode/verify-no-telemetry.sh` — each HARD hit is now classified per-file against
  ...
  verdict emits `RESULT: PASS — GlyphCode introduces ZERO telemetry; N vendored-dormant hits,
...
- `build/glyphcode/vendored-telemetry-allowlist.json` — **new** checked-in inventory: 19
...
- `build/glyphcode/telemetry-posture.md` — **new** posture doc (claim, verified-vs-assumed, the
  three vendored buckets, how the gate enforces, re-verify steps incl. the dynamic capture).

### Acceptance test (run + verified)

1. `bash build/glyphcode/verify-no-telemetry.sh ../VSCode-darwin-arm64/GlyphCode.app` →
...
2. **Strictness (negative test):** a synthetic target with a `dc.services.visualstudio.com`
   string injected into `extensions/glyphcode-trust-panel/dist/extension.js` (a GlyphCode
   surface) **and** into the allowlisted `extensions/git/dist/main.js` → the git file classifies
   VENDORED-DORMANT (passes), the glyphcode file classifies **UNEXPECTED** → **RESULT: FAIL**,
   exit 1. Confirms a GlyphCode-introduced (or any non-inventoried) telemetry string still fails.
...
- **Rebase risk: LOW.** All three are GlyphCode-only build tooling with no upstream counterpart.
...
- **Status:** `AUDIT` (2026-05-31). This is a discovery/classification deliverable, **not**
  ...
  egress code OUTSIDE GlyphCode's governance (or undermine the trust/trace claim) and
  classifies each honestly. The full register, rationale, and recommended hardening per
  milestone are in **`build/glyphcode/escape-hatch-audit.md`**; this section is the concise
  summary the patch ledger requires.

### Threat addressed

GlyphCode's claim is that it governs an agent's egress (metadata-only proxy) and produces an
...
**The fork does NOT patch the stock Code-OSS execution surfaces.** Diffing `glyphcode` from its
first GlyphCode commit, the GlyphCode `src/vs/**` changes are limited to branding/getting-started,
...
the `code`/`glyphcode` CLI, Remote-Tunnels / `serve-web`, settings sync, and the webview host are
UNMODIFIED stock Code-OSS.** GlyphCode's governance (egress proxy, sanitized env, trace, signed
verdict) lives entirely in the **first-party extension + supervisor** and governs ONLY the
runs/terminals/chat the extension itself creates. **Net: the GlyphCode envelope is a per-run /
...
1. **A1 — the un-governed stock integrated terminal inside a "GlyphCode IDE."** Governed env
...
- **CIO-FLAG-1 — "GlyphCode IDE" is a per-run envelope inside a stock IDE, not a confined IDE.**
  P0 can ship honestly only with explicit messaging that GlyphCode governs the runs/terminals it
...
governance), not a redirectable GlyphCode control.

### Files

- **`build/glyphcode/escape-hatch-audit.md`** — the full A–F register (vector, reachable?,
...
- **Rollback:** delete `build/glyphcode/escape-hatch-audit.md` and this section. No app/runtime
  impact (docs only).
...
## PATCH-005 — GlyphCode default theme + title/activity-bar chrome restyle (product look)

- **Status:** `APPLIED` (2026-06-01).
- **Type:** **UX / product-look patch.** Threat addressed = **N/A.** This is the GlyphCode
  default color theme + a light title-bar / activity-bar restyle so the workbench reads as
  GlyphCode (not stock Code-OSS), the way Cursor ships its own theme/chrome. Recorded here only
...
1. **Default color theme** — a new builtin theme extension `extensions/glyphcode-theme/`
   contributes `GlyphCode Dark` (`uiTheme: vs-dark`). Colors are derived from the design source
   `design/ide-concept/glyphcode-workbench-blended.html` `:root` tokens (slate substrate
   ...
   **desktop** by pointing `ThemeSettingDefaults.COLOR_THEME_DARK` at `'GlyphCode Dark'`
   ...
   splash (`COLOR_THEME_DARK_INITIAL_COLORS`, same file) is re-paletted to GlyphCode so a clean
   first launch never flashes stock chrome. For **web** (and as documented intent) it is also set
   via `product.json` → `configurationDefaults["workbench.colorTheme"] = "GlyphCode Dark"`, and
   `GlyphCode Dark` is added to `product.json` `onboardingThemes` as the lead dark choice.
...
3. **Activity-bar restyle** — softens the divider to the theme hairline, drops the stock shadow,
   ...
   tokens, inheriting the GlyphCode halo-blue from the theme). Width stays the layout-driven
...
- `extensions/glyphcode-theme/package.json` — **new** builtin theme extension manifest
  (`contributes.themes` → `GlyphCode Dark`).
- `extensions/glyphcode-theme/package.nls.json` — **new** localized display strings.
- `extensions/glyphcode-theme/themes/glyphcode-dark-color-theme.json` — **new** the color theme
...
- `src/vs/workbench/services/themes/common/workbenchThemeService.ts` — `ThemeSettingDefaults.COLOR_THEME_DARK`
  → `'GlyphCode Dark'` (the desktop default mechanism) + `COLOR_THEME_DARK_INITIAL_COLORS`
  re-paletted to GlyphCode for the boot splash.
- `product.json` — default `workbench.colorTheme` via `configurationDefaults` (web path);
  `GlyphCode Dark` added as the lead `onboardingThemes` dark entry. (`nameLong`/`nameShort`/branding
  already GlyphCode; window title `${appName}` already resolves to GlyphCode.)
- `src/vs/workbench/browser/parts/titlebar/media/titlebarpart.css` — appended GlyphCode title-bar
  restyle block.
- `src/vs/workbench/browser/parts/activitybar/media/activitybarpart.css` — appended GlyphCode rail
  part restyle block.
- `src/vs/workbench/browser/parts/activitybar/media/activityaction.css` — appended GlyphCode rail
  icon accent block.

### How to verify it is the default

1. Fresh profile / first run of the packaged build: the workbench loads in **GlyphCode Dark**
   without the user picking a theme (driven by `product.json` `configurationDefaults`).
2. Command Palette → "Preferences: Color Theme" lists **GlyphCode Dark** and shows it selected.
...
1. Launch the rebuilt app on a clean profile → theme is GlyphCode Dark, editor bg `#0f1419`,
...
- **Rollback:** delete `extensions/glyphcode-theme/`, revert the two `product.json` lines, and
  remove the three appended `/* GlyphCode … restyle */` CSS blocks. No runtime/security impact.
...
## PATCH-006 — Suppress the upstream "Agents window" from the GlyphCode UI

- **Status:** `APPLIED` (2026-06-01).
- **Type:** **Trust-honesty / product-surface patch.** Removes every UI path to Microsoft's upstream
  Agent Sessions window (`vs/sessions`) so it cannot be reached or advertised from the GlyphCode UI.
...
**un-governed**: entirely outside GlyphCode's trust/trace/verifier envelope (the per-run-envelope
reality in the M4 audit / CIO-FLAG-1). Left live, it **self-advertises** ("Try out the new Agents
window") an agent experience a user could mistake for *the* GlyphCode governed surface — a **false
...
- **Rebase risk: LOW.** All edits are commented-out registrations / a `when: false` in two
  GlyphCode-owned diff sites; if upstream adds a new Agents-window entry point, suppress it the
  same way.
...
- **Type:** **Trust-honesty / de-Copilot brand patch.** Removes the stock `$(copilot)` "Copilot
  Status" status-bar indicator so no Copilot icon/branding shows in the GlyphCode chrome. A
  `src/vs/**` workbench diff, hence logged.

### Threat addressed

GlyphCode de-Copilot'd its agent stack (GATE-002) and ships a first-party Codex-backed chat. But
...
visible right next to the GlyphCode policy chip. A Copilot icon/brand in a de-Copilot'd **trust**
...
`product.defaultChatAgent` is absent. The GlyphCode build configures none, so the Copilot
...
The GlyphCode Codex chat is a separate `ChatViewPane` view container
...
1. Launch the rebuilt app → no `$(copilot)` icon and no "Copilot Status"/"Copilot Sign In" text in
   the status bar.
2. The GlyphCode policy chip is still present; the native GlyphCode (Codex) chat view still opens
   and works.
...
A workbench contribution (`GlyphCodeTierChromeContribution`) observes the `glyphcode.tier`
**context-key** the GlyphCode first-party extension sets on every effective-tier change
(`extension/src/extension.ts` `setContext('glyphcode.tier', <tier>)`; values
`ask|inline|governed|sensitive|sovereign`; the Trust Panel webview posts `glyphcodeTier` →
`media/live.js` `postTier`). It mirrors that value to a `data-glyphcode-tier` attribute on the
...
(`glyphcodePromote`) remains the only authority door; this patch publishes a view cue, not a grant.
...
`glyphcode.authority`, and the halo never reads `glyphcode.tier`. The recede CSS touches only
...
GlyphCode Trust Panel webview all stay fully legible at Ask (the ladder + the Ask affordance must
remain bright).

### Accessibility

Honors **both** the OS `prefers-reduced-motion` (CSS media query) **and** the shared
`glyphcode-halo-reduce-motion` class the Authority Halo contribution toggles from the
`glyphcode.workbench.haloMotion` setting: either drops the `0.5s` cross-fade and keeps the **static**
...
- `glyphcode.workbench.askEditorRecede` (`boolean`, default **ON**, `APPLICATION` scope). When OFF
  the contribution adds a `glyphcode-tier-recede-off` opt-out class the CSS uses to suppress the
  recede; the `data-glyphcode-tier` attribute still reflects the friction axis for any future cue.
  Registered under the same `glyphcode` configuration id as the halo's `glyphcode.workbench.*`
  settings so they group in the Settings UI.

### Changed files

- `src/vs/workbench/browser/parts/glyphcodeTierChrome.ts` — **new** contribution
  (`GlyphCodeTierChromeContribution`, registered `registerWorkbenchContribution2` @
  `WorkbenchPhase.AfterRestored`) + the `glyphcode.workbench.askEditorRecede` setting registration.
- `src/vs/workbench/browser/parts/media/glyphcodeTierChrome.css` — **new** the editor-recede
...
- (Extension side, not a fork diff:) `extension/src/extension.ts` mirrors the webview's
  `glyphcodeTier` message to `setContext('glyphcode.tier', …)`; `extension/media/live.js` posts the
...
2. Open the GlyphCode Trust Panel and slide the Authority Ladder to **Ask** (or focus a run whose
...
4. With OS "reduce motion" on **or** `glyphcode.workbench.haloMotion: false`, no cross-fade — the
   static receded state remains at Ask.
5. Toggle `glyphcode.workbench.askEditorRecede: false` — the editor stays fully legible at Ask (the
...
- **Rollback:** delete `src/vs/workbench/browser/parts/glyphcodeTierChrome.ts` +
  `…/media/glyphcodeTierChrome.css` and remove the one import line in `workbench.common.main.ts`. The
  `data-glyphcode-tier` attribute and the `glyphcode.workbench.askEditorRecede` setting then
...
- **Status:** `APPLIED` (2026-06-02).
- **Type:** **UX / product-look patch.** Threat addressed = **N/A**. The title-bar member of the GlyphCode chrome trio: the Authority Halo (PATCH-003) paints the *assurance* ring, the Tier Chrome (PATCH-008) recedes the editor at Ask, and this promotes the five-rung **friction** Authority Ladder (`design/ide-concept/BLENDED-WORKBENCH-SPEC.md` §5.2.2/§6) out of the Trust Panel webview into the title bar as the canonical friction control. Recorded here because it is a `src/vs/**` workbench diff (ledger policy logs every workbench diff) — but it is **not** a security patch: it only **reads** two existing context-keys, **invokes** a command, and **paints chrome**. It cannot change trust state, grant authority, mint a verdict, or imply assurance.

### What it does
Adds a native title-bar widget in `.titlebar-center`: an actor-mini chip, a five-rung radiogroup (`ask · inline · governed · sensitive · sovereign`), and a level chip. The active rung is driven by the `glyphcode.tier` context-key; the level chip + active-rung tint borrow the `glyphcode.authority` halo color (tint only) and carry the §3.4 non-color glyph+label. Clicking/keying a rung invokes the `glyphcode.setTier` command (view-only friction change). Behind `glyphcode.workbench.titleLadder` (default ON).

### Mechanism (read two keys, invoke one command — it does not derive or write trust)
A workbench contribution (`GlyphCodeTitleLadderContribution`, `registerWorkbenchContribution2` @ `AfterRestored`) mounts the widget into the title-bar part's `.titlebar-center` (`IWorkbenchLayoutService.getContainer(mainWindow, Parts.TITLEBAR_PART)`), re-mounting idempotently on title-bar visibility changes + self-healing if the center is recreated. It READS `glyphcode.tier` (active rung) and `glyphcode.authority` (level chip color/label, tint only) via `onDidChangeContext`. On a rung click it INVOKES `glyphcode.setTier` via `ICommandService` — it NEVER writes a context-key. The extension's `glyphcode.setTier` command is the single writer of `glyphcode.tier` (via a `TierController`), which also syncs the Trust Panel webview ladder. One source, two views.

### Orthogonality (the two-axis honesty invariant)
This is the **friction** axis; the halo (PATCH-003) is the **assurance** axis (§2.1). The widget never reads or writes `glyphcode.authority` except to borrow its color for tint, and never sets assurance from the tier. The promote modal remains the only authority door (§6/§2.4 "tier is enforced authority, not a UI hint" — the ladder reflects a grant, it does not confer one).
...
### Accessibility
`role=radiogroup`/`radio`, one `aria-checked` + one roving `tabindex=0`, the full keyboard map (arrows/Home/End/Space/Enter), a visible focus ring, the §3.4 non-color glyph+label on the level chip, and reduced-motion honored via both the OS preference and the shared `glyphcode-halo-reduce-motion` class (owned by the halo; consumed here in CSS only).

### Setting
- `glyphcode.workbench.titleLadder` (`boolean`, default **ON**, `APPLICATION` scope). Registered under the same `glyphcode` config id as the halo/tier-chrome settings so they group.

### Changed files
- `src/vs/workbench/browser/parts/glyphcodeTitleLadder.ts` — **new** contribution + the `glyphcode.workbench.titleLadder` setting.
- `src/vs/workbench/browser/parts/media/glyphcodeTitleLadder.css` — **new** ladder/actor-mini/level-chip styling + assurance→tint map + reduced-motion rules.
- `src/vs/workbench/workbench.common.main.ts` — one import line wiring the contribution in.
- (Extension side, not a fork diff:) `extension/src/extension.ts` adds the `glyphcode.setTier` command + a `TierController` (the single `glyphcode.tier` writer) that syncs the webview; `extension/media/live.js` adds the `setTier` receiver (`applyHostTier`, no-echo loop-breaker).
...
2. Click a title-bar rung (e.g. **Ask**): the active rung moves, the `glyphcode.tier` key flips, the editor recedes (PATCH-008), and — if the Trust Panel is open — its webview ladder moves to the same rung. No authority is granted (no promote modal fires).
...
5. With OS "reduce motion" on **or** `glyphcode.workbench.haloMotion: false`: rung/chip transitions are static.
6. Toggle `glyphcode.workbench.titleLadder: false`: the widget is removed from the title bar; the `glyphcode.tier` key still tracks for the editor-recede chrome.
...
### Rollback
Remove the one import line in `workbench.common.main.ts` and delete `glyphcodeTitleLadder.ts` + `glyphcodeTitleLadder.css`. The extension's `glyphcode.setTier` command + `TierController` become inert (the webview ladder still drives the tier through the same controller). Fully reversible; no other fork file references it.
