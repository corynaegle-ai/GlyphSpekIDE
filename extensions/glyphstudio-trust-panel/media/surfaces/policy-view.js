/*
 * GlyphStudio POLICY-as-code rail surface — webview renderer (Spec §5.9).
 *
 * Receives two message kinds from the provider (policyViewProvider.ts):
 *   - { type:'policy/posture',  posture }   — the standing .glyphstudio/policy.yml posture
    ...
    card.appendChild(
      el('div', {
        ...
        text: posture.fromSetting
          ? 'from glyphstudio.policyPath setting: ' + posture.settingValue
          : 'default (glyphstudio.policyPath unset): ' + posture.settingValue,
      }),
    );
