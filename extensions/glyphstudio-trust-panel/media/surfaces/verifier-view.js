/*
 * GlyphStudio INDEPENDENT VERIFIER rail surface — webview renderer (Spec §5.9 / §5.8 /
 ...
 *   (window.GLYPHSTUDIO_TRUSTED_VERIFIER_KEYS — the SAME trust root the Trust Panel
...
(function () {
   ...
   * SAME set the Trust Panel pins (operator `glyphstudio.trustedVerifierKeys` + the
  ...
  function trustedVerifierKeys() {
    const injected =
      typeof window !== 'undefined' && Array.isArray(window.GLYPHSTUDIO_TRUSTED_VERIFIER_KEYS)
        ? window.GLYPHSTUDIO_TRUSTED_VERIFIER_KEYS
        : [];
    const ids = new Set();
    const keys = [];
  ...
  function rec(runId) {
    ...
    if (!r) {
      r = {
        ...
        liveTraceRoot: null,
        sig: null,
        // The CANONICAL gate result fanned from the Trust Panel (glyphstudioAuthority →
  ...
  function renderTargets() {
    ...
    if (!checks) {
      root.appendChild(el('div', 'targets-empty',
        'No verification targets observed yet. Targets are declared in .glyphstudio/policy.yml (Policy.verify) and surface here when a governed run produces a verdict.'));
      return;
    }
