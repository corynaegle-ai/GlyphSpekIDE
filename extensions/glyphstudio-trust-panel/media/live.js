/*
 * GlyphStudio LIVE Trust Panel (milestone M5) — webview side.
 *
...
(function () {
  ...
  // bridgeProtocol's RUN_TRUSTS as window.GLYPHSTUDIO_RUN_TRUSTS (a nonce-guarded
  ...
  var RUN_TRUSTS = (typeof window !== 'undefined' && Array.isArray(window.GLYPHSTUDIO_RUN_TRUSTS))
    ? window.GLYPHSTUDIO_RUN_TRUSTS
  ...
  // ladder does NOT echo `glyphstudioTier` straight back to the host — the host already
  ...
  function postAuthority(runId, authority) {
    ...
    if (typeof window !== 'undefined' && typeof window.GLYPHSTUDIO_POST === 'function') {
      window.GLYPHSTUDIO_POST({ type: 'glyphstudioAuthority', runId: runId, authority: authority });
    }
  }
   ...
   * host mirrors it to the `glyphstudio.tier` context-key the fork chrome reads to
  ...
  function postTier(tier) {
    ...
    lastPostedTier = tier;
    if (typeof window !== 'undefined' && typeof window.GLYPHSTUDIO_POST === 'function') {
      window.GLYPHSTUDIO_POST({ type: 'glyphstudioTier', tier: tier });
    }
  }
   ...
   * the webview's own rungs use, but suppresses the `glyphstudioTier` echo so there is no
  ...
  function renderAbrHeader(review) {
    ...
    badges.appendChild(el('span', {
      ...
      title: 'GlyphStudio governs and traces this run (egress observed, trace signed) but does not contain it. The verdict is honest evidence, never a product-trusted guarantee.',
    }));
  ...
  function submitAbrDecision(decision) {
    ...
    if (typeof window !== 'undefined' && typeof window.GLYPHSTUDIO_POST === 'function') {
      window.GLYPHSTUDIO_POST({ type: 'agenticBuildDecision', runId: abrCurrentRunId, decision: decision });
    }
    var root = document.getElementById('abr-decision-body');
    if (root) {
      clear(root);
  ...
  function mockScenarios(runId, scenario) {
    ...
    base.push(opened);
    base.push(mockTraceEvent(runId, 0, '', 'run_created', { runId: runId, runDir: '.glyphstudio/runs/' + runId, provenanceLabel: 'user' }));
   ...
   * `glyphstudio-reduce-motion` class on <html> (styles.css shares the OS-pref rule
   * list with that class). This is the webview-readable analog of the fork's
   * `glyphstudio.workbench.haloMotion` setting: it gives the Trust Panel the SECOND
  ...
  function applyReduceMotion(on) {
    ...
    document.documentElement.classList.toggle('glyphstudio-reduce-motion', on === true);
  }

  /* --------------------------- wiring --------------------------- */
  function wireLive() {
    // Reduced-motion Settings toggle (§14.3): seed from a host-injected global if the
    // host set one BEFORE this script ran (mirrors GLYPHSTUDIO_RUN_TRUSTS / trusted-key
    // injection), then keep it live via the `reduceMotion` host message below.
    if (typeof window !== 'undefined' && typeof window.GLYPHSTUDIO_REDUCE_MOTION !== 'undefined') {
      applyReduceMotion(window.GLYPHSTUDIO_REDUCE_MOTION === true);
    }
    ...
    window.addEventListener('message', function (event) {
      ...
      if (msg.type === 'runEvent' && msg.event) {
        ...
        // §14.3 Settings-toggle path: the host posts the user's GlyphStudio "reduce
        ...
        // it. Applied WITHOUT echoing `glyphstudioTier` back (applyHostTier suppresses the
    ...
    // PROMOTE TO GOVERNED RUN (§5.8, §6). The button posts `glyphstudioPromote` to the
    // host, which runs glyphstudio.promoteChatToBuild — the first-party agentic-build
    ...
    if (promoteBtn) {
      promoteBtn.addEventListener('click', function () {
        if (typeof window !== 'undefined' && typeof window.GLYPHSTUDIO_POST === 'function') {
          window.GLYPHSTUDIO_POST({ type: 'glyphstudioPromote' });
        }
      });
    }
    ...
    // governed run ▸" is the EXPLICIT promotion entry point: it posts glyphstudioPromote
    // → glyphstudio.promoteChatToBuild, whose OWN modal is the authority gate. Neither
    ...
    window.GLYPHSTUDIO_LIVE = {
      ...
      // injected window.GLYPHSTUDIO_RUN_TRUSTS, or the fail-safe mirror). Exposed so
      ...
      // assert the de-duplicated `glyphstudioTier` post on the REAL panel code. The
      // host mirrors it to the `glyphstudio.tier` context-key the fork chrome reads.
      postTier: postTier,
      ...
      // echoing `glyphstudioTier` back (the loop-breaker), on the REAL panel code.
      ...
      // Settings-toggle path adds/removes the `glyphstudio-reduce-motion` class.
