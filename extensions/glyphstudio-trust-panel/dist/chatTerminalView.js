"use strict";
/*
 * GlyphStudio CHAT TERMINAL VIEW (M7 — "chat that's a terminal").
 *
 * A WebviewViewProvider for the activity-bar `glyphstudio.chat` view that hosts an
 ...
 * own TUI provides the conversation; GlyphStudio provides the governed frame.
 *
 ...
 * GlyphStudio injects NO credential and sets NO base-url override (anti-FauxCode). The
...
class ChatTerminalViewProvider {
    ...
    async start() {
        ...
        try {
            ...
            if (!ptyMod) {
                this.post({
                    type: 'error',
                    message: 'GlyphStudio Chat needs a PTY backend (node-pty) to run your interactive Claude Code. ' +
            ...
            this.output.appendLine(`[chat] governed PTY started — run ${session.runId}, binary ${session.launchPath}. ` +
                'Governed (metadata-only egress) + traced; UNSANDBOXED; GlyphStudio holds no key.');
        }
        catch (err) {
            const message = `GlyphStudio Chat could not start: ${String(err?.message ?? err)}`;
     ...
     * the GlyphStudio logo. Scripts run only with the nonce; assets only from media/.
     */
    getHtml(webview) {
...
ChatTerminalViewProvider.viewType = 'glyphstudio.chat';
