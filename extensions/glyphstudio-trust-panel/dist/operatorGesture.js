"use strict";
/*
 * GlyphStudio FIRST-PARTY OPERATOR-GESTURE TOKEN (sweep-19 High #3).
 *
 * THE TRUST PROBLEM. `glyphstudio.runGovernedTask` is contributed in package.json
 * and registered with vscode.commands.registerCommand, so it is GLOBALLY invokable
 * by ANY extension via vscode.commands.executeCommand('glyphstudio.runGovernedTask').
 ...
 * executeCommand('glyphstudio.runGovernedTask') re-enters OUR handler, which DOES
 ...
 * executeCommand('glyphstudio.runGovernedTask') from OUTSIDE the first-party gesture
