"use strict";
/*
 * GlyphStudio EVIDENCE-GROUNDED AI COMMIT MESSAGE — the vscode-facing command.
 *
 * "GlyphStudio: Generate Commit Message" (SCM input sparkle + command palette). It pre-fills the
 ...
 *     (the `glyphstudio.trustedVerifierKeys` setting + the machine keystore public key) AND, only
 ...
 * ============================================================== */
const MSG = {
    notARepo: 'GlyphStudio: no Git repository here — open a folder under version control to generate a commit message.',
    nothingStaged: 'GlyphStudio: nothing is staged. Stage the changes you want to commit first (unstaged work is not used).',
    noGitExtension: 'GlyphStudio: the built-in Git extension is unavailable, so a commit message cannot be generated.',
    gatewayFailed: (detail) => `GlyphStudio: the governed gateway could not generate a message (${detail}). Your commit input was left unchanged.`,
    emptyReply: 'GlyphStudio: the model returned no usable message. Your commit input was left unchanged.',
    cancelled: 'GlyphStudio: commit-message generation cancelled. Your commit input was left unchanged.',
    inFlight: 'GlyphStudio: a commit message is already being generated for this repository.',
    replaceConfirm: 'Your commit message box already has text. Replace it with the generated message?',
    replaceYes: 'Replace',
    replaceKeep: 'Keep mine',
    progressTitle: 'GlyphStudio: generating commit message…',
    noTrailerNote: (reason) => `GlyphStudio: ${reason}`,
};
...
function trustedVerifierKeys() {
    ...
    try {
        const keystore = path.join(homeDir(), '.glyphstudio', 'verifier', 'public.pem');
 ...
 * shape (the machine `glyphstudio.runOutputRoot` setting, else ~/.glyphstudio/runs), namespaced by
...
function resolveRunsBaseForCommit() {
    let root;
    try {
        const configured = vscode.workspace.getConfiguration('glyphstudio').get('runOutputRoot', '');
        root = configured && configured.trim() ? configured.trim() : path.join(homeDir(), '.glyphstudio', 'runs');
    }
    catch {
        root = path.join(homeDir(), '.glyphstudio', 'runs');
    }
    if (!root)
        return undefined;
...
/**
 * Register `glyphstudio.generateCommitMessage`. WIRE this in extension.ts activate() next to the
...
function registerGenerateCommitMessage(context, deps) {
    ...
    const disposable = vscode.commands.registerCommand('glyphstudio.generateCommitMessage', (arg) => runGenerate(arg, deps, discover));
...
function verifiedTrailerEnabled() {
    try {
        return vscode.workspace.getConfiguration('glyphstudio').get('commitMessage.verifiedTrailer', true);
    }
    catch {
        return true;
    }
...
function conventionalCommitsEnabled() {
    try {
        return vscode.workspace.getConfiguration('glyphstudio').get('commitMessage.conventionalCommits', false);
    }
    catch {
        return false;
    }
