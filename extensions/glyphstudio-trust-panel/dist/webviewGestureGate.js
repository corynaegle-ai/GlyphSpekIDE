"use strict";
/*
 * GlyphStudio FIRST-PARTY WEBVIEW GESTURE GATE (sweep-20 High #3 — rework of sweep-19).
 *
 * WHY THE COMMAND-HANDLER MINT WAS INSUFFICIENT. The sweep-19 fix minted the
 * operator-gesture token INSIDE the globally-invokable `glyphstudio.runGovernedTask`
 * command handler. VS Code commands carry NO trustworthy caller attribution, so a
 * third-party extension that calls
 * `vscode.commands.executeCommand('glyphstudio.runGovernedTask')` RE-ENTERS our
 ...
 * `glyphstudio.bridgeSupervisedRun` / `glyphstudio.startLiveRun` commands (no gate at
 ...
 * operator clicked a button INSIDE the GlyphStudio Trust Panel webview, which posted
