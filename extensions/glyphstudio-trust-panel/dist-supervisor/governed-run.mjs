...
function buildReadme(info) {
  return `# GlyphStudio P0 AUTONOMOUS Capstone Bundle

This directory is the verified evidence bundle for one **fully autonomous**
governed-and-verified run of the GlyphStudio P0 spike against \`demo-repo\` (a
...
function effectiveRunsBase(args) {
  return args.runsBase ?? process.env.GLYPHSTUDIO_RUNS_BASE ?? path6.join(os.homedir(), ".glyphstudio", "runs");
}
...
function buildVerifyOnlyReadme(info) {
  ...
  return `# GlyphStudio Verified Bundle (verify-only)

This directory is the verified evidence bundle for one **verify-only** governed
run of GlyphStudio against \`${info.repoPath}\`.
