/*
 * GlyphSpek INDEPENDENT VERIFIER rail surface — webview renderer (Spec §5.9 / §5.8 /
 ...
 *   (window.GLYPHSPEK_TRUSTED_VERIFIER_KEYS — the SAME trust root the Trust Panel
...
(function () {
   ...
   * SAME set the Trust Panel pins (operator `glyphspek.trustedVerifierKeys` + the
  ...
  function renderTargets() {
    ...
    if (!checks) {
      root.appendChild(el('div', 'targets-empty',
        'No verification targets observed yet. Targets are declared in .glyphspek/policy.yml (Policy.verify) and surface here when a governed run produces a verdict.'));
      return;
    }
