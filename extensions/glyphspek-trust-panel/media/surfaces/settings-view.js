/*
 * GlyphSpek SETTINGS rail surface — webview renderer (Spec §5.9 "Settings").
 *
 ...
 * NO validation and INVENTS no state: the host read the ACTUAL glyphspek.* config and
...
(function () {
  ...
  function renderIntro() {
    ...
    els.intro.appendChild(
      el(
        'div',
        null,
        'These are your LOCAL glyphspek.* settings — not the run/event stream. Toggling a ' +
  ...
  function renderFoot() {
    els.foot.textContent =
      'Reflects your actual glyphspek.* configuration. Only wired settings appear; ' +
