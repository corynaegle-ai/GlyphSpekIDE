# GlyphSpek Trust Panel — IDE extension (Workstream 4)

The GlyphSpek **Trust Panel** hosted as a **plain VS Code / Code-OSS extension**.
...
2. **Signature-before-display gate, against an out-of-band trust root.** A verifier
   ...
   (the `glyphspek.trustedVerifierKeys` machine-scoped setting, or the extension
   keystore at `~/.glyphspek/verifier/`), **never** a key carried inside the run
...
- **GlyphSpek: Open Trust Panel** (`glyphspek.openTrustPanel`) — opens the panel.
  On open it loads and **verifies** the embedded signed sample bundle in-browser.
- **GlyphSpek: Load Run Bundle…** (`glyphspek.loadRunBundle`) — pick a run-bundle
...
- **GlyphSpek: Run Governed Task…** (`glyphspek.runGovernedTask`) — **development
  ...
  verified against the verifier key pinned out-of-band from `~/.glyphspek/verifier/`.
  ...
  currently requires a local spikes checkout (set `glyphspek.supervisorPath` in
...
5. Run **GlyphSpek: Open Trust Panel** from the Command Palette. It opens with the
   signed sample already verified. Use **Tamper sample (demo)** to watch the gate
   flip the verdict to UNTRUSTED.
6. Run **GlyphSpek: Load Run Bundle…** and pick a real bundle directory — see
   below.
...
root (the extension keystore or `glyphspek.trustedVerifierKeys`). A bundle whose key
...
```
npx @vscode/vsce package          # produces glyphspek-0.0.1.vsix
npx ovsx publish glyphspek-0.0.1.vsix --pat <OPEN_VSX_TOKEN>
```
...
```
extension/
  package.json            # name "glyphspek", two commands, no Marketplace deps
