"use strict";
/*
 * GlyphSpek CHAT TERMINAL VIEW (M7 — "chat that's a terminal").
 *
 * A WebviewViewProvider for the activity-bar `glyphspek.chat` view that hosts an
 ...
 * own TUI provides the conversation; GlyphSpek provides the governed frame.
 *
 ...
 * GlyphSpek injects NO credential and sets NO base-url override (anti-FauxCode). The
...
class ChatTerminalViewProvider {
    ...
    async start() {
        ...
        try {
            ...
            if (!ptyMod) {
                this.post({
                    type: 'error',
                    message: 'GlyphSpek Chat needs a PTY backend (node-pty) to run your interactive Claude Code. ' +
            ...
            this.output.appendLine(`[chat] governed PTY started — run ${session.runId}, binary ${session.launchPath}. ` +
                'Governed (metadata-only egress) + traced; UNSANDBOXED; GlyphSpek holds no key.');
        }
        catch (err) {
            const message = `GlyphSpek Chat could not start: ${String(err?.message ?? err)}`;
     ...
     * the GlyphSpek logo. Scripts run only with the nonce; assets only from media/.
     */
    getHtml(webview) {
...
ChatTerminalViewProvider.viewType = 'glyphspek.chat';
