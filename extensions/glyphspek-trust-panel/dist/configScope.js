 ...
 *   - glyphspek.trustedVerifierKeys — a repo-supplied key would let a bundle's
 *     verdict reach AUTHORITATIVE under a key the operator never provisioned.
 *   - glyphspek.supervisorPath — a repo-supplied path would redirect the spawned,
 ...
 *
 * The keystore public key (~/.glyphspek/verifier/public.pem) is appended when
 ...
 * launched from here.
 *
 * @param inspect           getConfiguration('glyphspek').inspect('trustedVerifierKeys')
 ...
 *
 * Used for glyphspek.devSupervisorTrustKey: the gate that hands the verifier
 ...
 *
 * @param inspect   getConfiguration('glyphspek').inspect('devSupervisorTrustKey')
...
/**
 * Classify the `glyphspek.policyPath` setting from an inspect() result, in VS
 ...
 * supplied.
 *
 * @param inspect getConfiguration('glyphspek').inspect('policyPath')
 ...
 * a relative '.glyphspek-fixtures/capstone-policy.json' that would resolve
...
function resolveDefaultPolicyCandidate(supervisorPath, existsFn) {
    ...
    const candidate = path.join(supervisorPath, '.glyphspek-fixtures', 'capstone-policy.json');
