"use strict";
/*
 * GlyphSpek supervisor runner — extension-host bridge to the governed-run CLI.
 *
...
/**
 * The container-name prefix the docker-runtime uses (`glyphspek-<sanitized
 ...
 */
const CONTAINER_NAME_PREFIX = 'glyphspek-';
/**
 * Discover THIS run's container name from a runId.
 *
 * The docker-runtime derives the container name as `glyphspek-<runId>` after
...
function extractRunId(text) {
    ...
    const named = text.match(/glyphspek-([a-z0-9]{6,}-[0-9a-f-]{8,})/i);
    if (named)
        return named[1];
    return undefined;
}
 ...
 * `glyphspek-*` container, destroying unrelated/concurrent governed runs. We now
...
function sweepRunContainer(runId, output) {
    ...
    try {
        ...
        // removal — filter by the exact name, never the glyphspek- prefix.
...
function runSupervisor(opts) {
    return new Promise((resolve) => {
        ...
        output.appendLine('');
        output.appendLine(`[host] GlyphSpek governed run — mode: ${opts.mode} (${bundled ? 'bundled+pinned' : 'dev-override, un-pinned'})`);
        ...
        child.on('close', (code, signal) => {
            ...
            if (cancelled) {
                ...
                // never the whole glyphspek-* namespace (Medium #2).
