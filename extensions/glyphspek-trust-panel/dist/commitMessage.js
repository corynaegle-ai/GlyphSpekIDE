"use strict";
/*
 * GlyphSpek EVIDENCE-GROUNDED AI COMMIT MESSAGE — the vscode-facing command.
 *
 * "GlyphSpek: Generate Commit Message" (SCM input sparkle + command palette). It pre-fills the
 ...
 *     (the `glyphspek.trustedVerifierKeys` setting + the machine keystore public key) AND, only
 ...
 * ============================================================== */
const MSG = {
    notARepo: 'GlyphSpek: no Git repository here — open a folder under version control to generate a commit message.',
    nothingStaged: 'GlyphSpek: nothing is staged. Stage the changes you want to commit first (unstaged work is not used).',
    noGitExtension: 'GlyphSpek: the built-in Git extension is unavailable, so a commit message cannot be generated.',
    gatewayFailed: (detail) => `GlyphSpek: the governed gateway could not generate a message (${detail}). Your commit input was left unchanged.`,
    emptyReply: 'GlyphSpek: the model returned no usable message. Your commit input was left unchanged.',
    cancelled: 'GlyphSpek: commit-message generation cancelled. Your commit input was left unchanged.',
    inFlight: 'GlyphSpek: a commit message is already being generated for this repository.',
    replaceConfirm: 'Your commit message box already has text. Replace it with the generated message?',
    replaceYes: 'Replace',
    replaceKeep: 'Keep mine',
    progressTitle: 'GlyphSpek: generating commit message…',
    noTrailerNote: (reason) => `GlyphSpek: ${reason}`,
};
...
function trustedVerifierKeys() {
    ...
    try {
        const keystore = path.join(homeDir(), '.glyphspek', 'verifier', 'public.pem');
...
function resolveRunsBaseForCommit() {
    let root;
    try {
        const configured = vscode.workspace.getConfiguration('glyphspek').get('runOutputRoot', '');
        root = configured && configured.trim() ? configured.trim() : path.join(homeDir(), '.glyphspek', 'runs');
    }
    catch {
        root = path.join(homeDir(), '.glyphspek', 'runs');
    }
    if (!root)
        return undefined;
...
/**
 * Register `glyphspek.generateCommitMessage`. WIRE this in extension.ts activate() next to the
...
function registerGenerateCommitMessage(context, deps) {
    ...
    const disposable = vscode.commands.registerCommand('glyphspek.generateCommitMessage', (arg) => runGenerate(arg, deps, discover));
...
function verifiedTrailerEnabled() {
    try {
        return vscode.workspace.getConfiguration('glyphspek').get('commitMessage.verifiedTrailer', true);
    }
    catch {
        return true;
    }
...
function conventionalCommitsEnabled() {
    try {
        return vscode.workspace.getConfiguration('glyphspek').get('commitMessage.conventionalCommits', false);
    }
    catch {
        return false;
    }
