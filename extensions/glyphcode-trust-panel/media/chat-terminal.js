/*
 * GlyphCode CHAT TERMINAL — webview renderer.
 *
...
(function () {
  ...
  // GlyphCode-themed xterm palette (dark, matches the Trust Panel surface).
  const THEME = {
    background: '#0f1419',
    foreground: '#d7dde5',
    cursor: '#4b8fe0',
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (msg.type) {
      ...
      case 'exited':
        if (term) {
          term.write(
            '\r\n\x1b[2m[GlyphCode] session ended (exit ' + String(msg.code) + '). ' +
