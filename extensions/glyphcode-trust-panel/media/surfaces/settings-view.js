/*
 * GlyphCode SETTINGS rail surface — webview renderer (Spec §5.9 "Settings").
 *
 ...
 * NO validation and INVENTS no state: the host read the ACTUAL glyphcode.* config and
...
(function () {
  ...
  function renderIntro() {
    ...
    els.intro.appendChild(
      el(
        'div',
        null,
        'These are your LOCAL glyphcode.* settings — not the run/event stream. Toggling a ' +
  ...
  function renderFoot() {
    els.foot.textContent =
      'Reflects your actual glyphcode.* configuration. Only wired settings appear; ' +
