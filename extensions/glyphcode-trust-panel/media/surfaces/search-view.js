/*
 * GlyphCode SEARCH rail surface — webview renderer (Spec §5.9 "Search"), INTERIM build.
 *
...
(function () {
  ...
  function renderResults() {
    ...
    if (!rows.length) {
      els.results.appendChild(
        el('p', 'empty', 'No recorded-history records match. This searches what GlyphCode observed, not live sandbox files.'),
      );
      return;
    }
  ...
  function renderFoot() {
    els.foot.textContent =
      'INTERIM: searches this run’s recorded history (events GlyphCode observed and folded), ' +
