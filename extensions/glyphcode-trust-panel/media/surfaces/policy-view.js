/*
 * GlyphCode POLICY-as-code rail surface — webview renderer (Spec §5.9).
 *
 * Receives two message kinds from the provider (policyViewProvider.ts):
 *   - { type:'policy/posture',  posture }   — the standing .glyphcode/policy.yml posture
    ...
    card.appendChild(
      el('div', {
        ...
        text: posture.fromSetting
          ? 'from glyphcode.policyPath setting: ' + posture.settingValue
          : 'default (glyphcode.policyPath unset): ' + posture.settingValue,
      }),
    );
