"use strict";
/*
 * GlyphCode authenticated supervisor bridge — CLIENT side (M2).
 *
 ...
 * for the whole window. Overridable via the `glyphcode.index.buildTimeoutMs` setting.
 */
...
class SupervisorBridge {
     ...
     * ============================================================== */
    /**
     * Send ONE chat turn (`chat/send`) to the GlyphCode-controlled model gateway. The
    ...
    request(method, params, opts) {
        return new Promise((resolve) => {
            ...
            const envelope = {
                glyphcode: bridgeProtocol_1.BRIDGE_JSONRPC,
                id,
                method,
                params,
            };
    ...
    errorResponse(id, code, message) {
        return { glyphcode: bridgeProtocol_1.BRIDGE_JSONRPC, id, error: { code, message } };
    }
}
