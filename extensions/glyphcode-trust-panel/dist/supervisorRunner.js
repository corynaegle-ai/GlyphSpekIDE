"use strict";
/*
 * GlyphCode supervisor runner — extension-host bridge to the governed-run CLI.
 *
...
/**
 * The container-name prefix the docker-runtime uses (`glyphcode-<sanitized
 ...
 */
const CONTAINER_NAME_PREFIX = 'glyphcode-';
/**
 * Discover THIS run's container name from a runId.
 *
 * The docker-runtime derives the container name as `glyphcode-<runId>` after
...
function extractRunId(text) {
    ...
    const named = text.match(/glyphcode-([a-z0-9]{6,}-[0-9a-f-]{8,})/i);
    if (named)
        return named[1];
    return undefined;
}
 ...
 * `glyphcode-*` container, destroying unrelated/concurrent governed runs. We now
...
function sweepRunContainer(runId, output) {
    ...
    try {
        ...
        // removal — filter by the exact name, never the glyphcode- prefix.
...
function runSupervisor(opts) {
    return new Promise((resolve) => {
        ...
        output.appendLine('');
        output.appendLine(`[host] GlyphCode governed run — mode: ${opts.mode} (${bundled ? 'bundled+pinned' : 'dev-override, un-pinned'})`);
        ...
        child.on('close', (code, signal) => {
            ...
            if (cancelled) {
                ...
                // never the whole glyphcode-* namespace (Medium #2).
