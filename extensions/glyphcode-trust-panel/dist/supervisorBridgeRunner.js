"use strict";
/*
 * GlyphCode supervisor BRIDGE runner — extension-host launcher for the packaged,
 ...
 * GLYPHCODE_* knobs this runner sets. Mirrors supervisorRunner's allowlist so the
 ...
 * runtime-relevant allowlist, any COLIMA_* var, plus the two GLYPHCODE_* knobs the
...
function buildBridgeEnv(opts) {
    ...
    const env = {
        ...
        GLYPHCODE_RUNS_BASE: runsBase,
    };
    if (opts.supervisorVersion)
        env.GLYPHCODE_SUPERVISOR_VERSION = opts.supervisorVersion;
    ...
    if (opts.verifierKeyPath)
        env.GLYPHCODE_VERIFIER_KEY = opts.verifierKeyPath;
...
function defaultRunsBase() {
    return path.join(os.homedir(), '.glyphcode', 'runs');
}
/**
...
async function createRunViaBridge(opts) {
    ...
    output.appendLine('');
    output.appendLine('[host] GlyphCode supervisor bridge — spawning packaged bridge-server.');
    output.appendLine(`[host] bridge-server: ${opts.bridgeServerPath}`);
    output.appendLine(`[host] runs base: ${env.GLYPHCODE_RUNS_BASE}`);
...
async function startRunViaBridge(opts) {
    ...
    const env = buildBridgeEnv(opts);
    output.appendLine('');
    output.appendLine('[host] GlyphCode supervisor bridge — spawning packaged bridge-server (LIVE run).');
    output.appendLine(`[host] bridge-server: ${opts.bridgeServerPath}`);
    output.appendLine(`[host] runs base: ${env.GLYPHCODE_RUNS_BASE}`);
    const bridge = new bridge_1.SupervisorBridge({
        binaryPath: opts.bridgeServerPath,
...
async function startGovernedTerminalSession(opts) {
    ...
    const env = buildBridgeEnv(opts);
    output.appendLine('');
    output.appendLine('[host] GlyphCode governed terminal — spawning packaged bridge-server.');
    output.appendLine(`[host] bridge-server: ${opts.bridgeServerPath}`);
    output.appendLine(`[host] runs base: ${env.GLYPHCODE_RUNS_BASE}`);
    const bridge = new bridge_1.SupervisorBridge({
        binaryPath: opts.bridgeServerPath,
...
async function openChatSession(opts) {
    ...
    const env = buildBridgeEnv(opts);
    output.appendLine('');
    output.appendLine('[host] GlyphCode native chat — spawning packaged bridge-server.');
    output.appendLine(`[host] bridge-server: ${opts.bridgeServerPath}`);
    output.appendLine(`[host] runs base: ${env.GLYPHCODE_RUNS_BASE}`);
    const bridge = new bridge_1.SupervisorBridge({
        binaryPath: opts.bridgeServerPath,
...
async function runAgenticBuild(opts) {
    ...
    const env = buildBridgeEnv(opts);
    output.appendLine('');
    output.appendLine('[host] GlyphCode governed agentic build — spawning packaged bridge-server.');
    output.appendLine(`[host] bridge-server: ${opts.bridgeServerPath}`);
