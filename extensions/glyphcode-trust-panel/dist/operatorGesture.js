"use strict";
/*
 * GlyphCode FIRST-PARTY OPERATOR-GESTURE TOKEN (sweep-19 High #3).
 *
 * THE TRUST PROBLEM. `glyphcode.runGovernedTask` is contributed in package.json
 * and registered with vscode.commands.registerCommand, so it is GLOBALLY invokable
 * by ANY extension via vscode.commands.executeCommand('glyphcode.runGovernedTask').
 ...
 * executeCommand('glyphcode.runGovernedTask') re-enters OUR handler, which DOES
 ...
 * executeCommand('glyphcode.runGovernedTask') from OUTSIDE the first-party gesture
