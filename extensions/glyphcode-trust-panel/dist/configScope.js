 ...
 *   - glyphcode.trustedVerifierKeys — a repo-supplied key would let a bundle's
 *     verdict reach AUTHORITATIVE under a key the operator never provisioned.
 *   - glyphcode.supervisorPath — a repo-supplied path would redirect the spawned,
 ...
 *
 * The keystore public key (~/.glyphcode/verifier/public.pem) is appended when
 ...
 * launched from here.
 *
 * @param inspect           getConfiguration('glyphcode').inspect('trustedVerifierKeys')
 ...
 *
 * Used for glyphcode.devSupervisorTrustKey: the gate that hands the verifier
 ...
 *
 * @param inspect   getConfiguration('glyphcode').inspect('devSupervisorTrustKey')
...
/**
 * Classify the `glyphcode.policyPath` setting from an inspect() result, in VS
 ...
 * supplied.
 *
 * @param inspect getConfiguration('glyphcode').inspect('policyPath')
 ...
 * a relative '.glyphcode-fixtures/capstone-policy.json' that would resolve
...
function resolveDefaultPolicyCandidate(supervisorPath, existsFn) {
    ...
    const candidate = path.join(supervisorPath, '.glyphcode-fixtures', 'capstone-policy.json');
