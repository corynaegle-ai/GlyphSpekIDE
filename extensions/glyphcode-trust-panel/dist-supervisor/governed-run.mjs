...
function buildReadme(info) {
  return `# GlyphCode P0 AUTONOMOUS Capstone Bundle

This directory is the verified evidence bundle for one **fully autonomous**
governed-and-verified run of the GlyphCode P0 spike against \`demo-repo\` (a
...
function effectiveRunsBase(args) {
  return args.runsBase ?? process.env.GLYPHCODE_RUNS_BASE ?? path6.join(os.homedir(), ".glyphcode", "runs");
}
...
function buildVerifyOnlyReadme(info) {
  ...
  return `# GlyphCode Verified Bundle (verify-only)

This directory is the verified evidence bundle for one **verify-only** governed
run of GlyphCode against \`${info.repoPath}\`.
...
async function runGovernedRunCli(argv) {
  let args;
  try {
    args = parseArgs(argv);
  } catch (err) {
    progress(err instanceof Error ? err.stack ?? err.message : String(err));
    emitResult({ error: "bad-args", message: err instanceof Error ? err.message : String(err) });
    return 2;
  }
  if (!dockerAvailable()) {
...
async function main() {
  process.exitCode = await runGovernedRunCli(process.argv.slice(2));
}
var isDirectRun = (() => {
  const entry = process.argv[1];
  if (!entry) return false;
  try {
    return path6.resolve(fileURLToPath4(import.meta.url)) === path6.resolve(entry);
  } catch {
    return false;
  }
})();
