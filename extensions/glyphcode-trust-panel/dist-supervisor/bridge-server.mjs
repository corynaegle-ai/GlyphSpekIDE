...
async function discoverFiles(opts) {
  ...
  const ignorePatterns = [
    ...
    ...await readRootIgnore(root, ".glyphcodeignore"),
...
var BRIDGE_JSONRPC = "glyphcode-jsonrpc/1";
var BridgeMethod = {
  ...
  /**
   * Send ONE chat turn to the GlyphCode-controlled model gateway (M7 chat — the
...
function isEnvelope(value) {
  return typeof value === "object" && value !== null && value.glyphcode === BRIDGE_JSONRPC;
}
...
var DEFAULT_MODEL_ENDPOINTS = [
  ...
  // `glyphcode govern -- claude` (TRUSTED posture; no --allow-ambient-env, no manual
  ...
  const checks = [
    {
      ...
      command: ["glyphcode", "governed-terminal", "verify"],
...
function resolveIndexResidency(opts) {
  ...
  switch (policy.residency) {
    ...
    case "workspace-encrypted": {
      ...
      const persistDir = resolve(root, ".glyphcode", "index");
    ...
    for (const src of req.contextSources ?? []) {
      if (src.ignored || this.isIgnored(src.ref)) {
        this.traceRedactions(runId, "context_source.ignored", [
          { field: src.ref, reason: "excluded by .glyphcodeignore" }
        ]);
        continue;
      }
...
function defaultKeyDir() {
  return join5(homedir(), ".glyphcode", "index-keys");
}
...
var DEFAULT_IMAGE = process.env.GLYPHCODE_SANDBOX_IMAGE ?? "node:22-alpine";
...
function buildAgenticBundleReadme(info) {
  ...
  return [
    "# GlyphCode verified-evidence bundle (governed agentic build)",
    "",
    `- run id: ${info.runId}`,
...
var VERIFY_OVERRIDE_PATH = ".glyphcode/verify.json";
    ...
    raw.on("connect", () => {
      raw.write(
        [
          ...
          `Host: ${hostAuthority}`,
          "User-Agent: GlyphCode-WebContext/1",
          "Connection: close",
          "",
          ""
        ].join("\r\n")
...
var BridgeServer = class {
      ...
      if (typeof maybeId === "number") {
        this.emit(
          this.errorResponse(
            ...
            "foreign envelope: missing or wrong glyphcode dialect tag"
          )
        );
      } else {
   ...
   * ============================================================== */
  /**
   * chat/send — drive ONE chat turn through the GlyphCode-controlled model
   ...
   * `glyphcode.index.persist` setting flowing through `index/build`'s params), build with
   ...
   *
   * RESIDENCY. `persist` (the `glyphcode.index.persist` setting carried in the params)
   ...
   * <workspaceRoot>/.glyphcode/index that survives a session restart).
   *
  ...
  successResponse(id, result) {
    return { glyphcode: BRIDGE_JSONRPC, id, result };
  }
...
var supervisorVersion = process.env.GLYPHCODE_SUPERVISOR_VERSION;
var runsBaseDir = process.env.GLYPHCODE_RUNS_BASE;
var verifierKeyPath = process.env.GLYPHCODE_VERIFIER_KEY;
