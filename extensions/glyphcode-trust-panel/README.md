# GlyphCode Trust Panel — IDE extension (Workstream 4)

The GlyphCode **Trust Panel** hosted as a **plain VS Code / Code-OSS extension**.
...
2. **Signature-before-display gate, against an out-of-band trust root.** A verifier
   ...
   (the `glyphcode.trustedVerifierKeys` machine-scoped setting, or the extension
   keystore at `~/.glyphcode/verifier/`), **never** a key carried inside the run
...
- **GlyphCode: Open Trust Panel** (`glyphcode.openTrustPanel`) — opens the panel.
  On open it loads and **verifies** the embedded signed sample bundle in-browser.
- **GlyphCode: Load Run Bundle…** (`glyphcode.loadRunBundle`) — pick a run-bundle
...
- **GlyphCode: Run Governed Task…** (`glyphcode.runGovernedTask`) — **development
  ...
  verified against the verifier key pinned out-of-band from `~/.glyphcode/verifier/`.
  ...
  currently requires a local spikes checkout (set `glyphcode.supervisorPath` in
...
5. Run **GlyphCode: Open Trust Panel** from the Command Palette. It opens with the
   signed sample already verified. Use **Tamper sample (demo)** to watch the gate
   flip the verdict to UNTRUSTED.
6. Run **GlyphCode: Load Run Bundle…** and pick a real bundle directory — see
   below.
...
root (the extension keystore or `glyphcode.trustedVerifierKeys`). A bundle whose key
...
```
npx @vscode/vsce package          # produces glyphcode-0.0.1.vsix
npx ovsx publish glyphcode-0.0.1.vsix --pat <OPEN_VSX_TOKEN>
```
...
```
extension/
  package.json            # name "glyphcode", two commands, no Marketplace deps
